...
namespace NetcodeIO.NET.Utils.IO
{
    ...
    internal sealed class UDPSocketContext : ISocketContext
    {
        ...
        public UDPSocketContext(AddressFamily addressFamily = AddressFamily.InterNetworkV6)
        {
            ...
            // CRITICAL FIX: Disable ICMP "port unreachable" cascade on Windows.
            // When a UDP packet is sent to a dead endpoint, Windows sends back an ICMP error
            // which normally causes WSAECONNRESET on the socket, killing ALL connections.
            // This flag tells Windows to silently ignore ICMP errors, allowing the socket
            // to remain healthy even when some peers die. Dead connections will timeout
            // naturally via keepalives rather than cascading to kill the entire mesh.
            const int SIO_UDP_CONNRESET = -1744830452; // The "magic number" for this Windows-specific fix
            try
            {
            ...
            {
                // This IOControl is Windows-only. Linux/Mac/iOS/Android don't have this behavior,
                // so it's safe (and expected) to fail on non-Windows platforms.
            }
