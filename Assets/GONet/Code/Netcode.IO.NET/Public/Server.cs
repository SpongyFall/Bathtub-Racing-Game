...
namespace NetcodeIO.NET
{
	...
	public class RemoteClient
	{
		...
		// Set to 1 when a disconnect is scheduled from a non-tick thread (e.g., parallel send flush).
		// Used to dedupe requests and prevent log/queue spam under failure conditions.
		internal int disconnectRequested;
	...
	public sealed class Server
	{
        ...
        internal void Start(bool autoTick)
        {
            ...
            if (autoTick)
            {
                // Use high-resolution Stopwatch instead of DateTime.UtcNow (15.6ms resolution on Windows)
                Stopwatch tickStopwatch = Stopwatch.StartNew();
                this.totalSeconds = tickStopwatch.Elapsed.TotalSeconds;

                Thread tickThread = new Thread(() => serverTick_SeparateThread(tickStopwatch));
		...
		private readonly ConcurrentQueue<RemoteClient> pendingDisconnects = new ConcurrentQueue<RemoteClient>();
		...
		internal void Tick(double newTotalSeconds)
		{
			...
			for (int i = 0; i < clientSlots.Length; i++)
			{
				...
				{
					clientSlots[i].Connected = false;
					clientSlots[i].Confirmed = false;
		...
		private void requestDisconnect(RemoteClient client, string context)
		{
			if (client == null)
				return;
			...
			// Only enqueue once per client to avoid churn under parallel send flush.
			if (Interlocked.Exchange(ref client.disconnectRequested, 1) != 0)
				return;

			pendingDisconnects.Enqueue(client);
			log("Disconnect scheduled for client {0}. Reason={1}", NetcodeLogLevel.Warning, client.RemoteEndpoint?.ToString() ?? "<null>", context);
		}

		private void processPendingDisconnects()
		{
			while (pendingDisconnects.TryDequeue(out var client))
			{
				if (client == null)
					continue;

				// Ensure this client is still tracked in a slot before notifying/disconnecting.
				bool found = false;
				for (int i = 0; i < clientSlots.Length; i++)
				{
					if (ReferenceEquals(clientSlots[i], client))
					{
						found = true;
						break;
					}
				...
				log("Client {0} disconnected (scheduled)", NetcodeLogLevel.Info, client.RemoteEndpoint?.ToString() ?? "<null>");
				disconnectClient(client);
			}
		}

		private void serverTick_SeparateThread(Stopwatch tickStopwatch)
		{
			// Use high-resolution Stopwatch for precise timing (microsecond precision)
			// Avoids DateTime.UtcNow's 15.6ms resolution on Windows which breaks 128Hz tick (7.8ms intervals)
			long tickDurationStopwatchTicks = Stopwatch.Frequency / tickrate; // High-res ticks per game tick

			while (isRunning)
			{
				long tickStartTicks = tickStopwatch.ElapsedTicks;

                try
                {
					double currentSeconds = tickStopwatch.Elapsed.TotalSeconds;
					Tick(currentSeconds);
                }
                catch (Exception e)
                {
                ...
                {
					// Calculate precise sleep duration using high-resolution stopwatch ticks
					long elapsedTicks = tickStopwatch.ElapsedTicks - tickStartTicks;
					long ticksToSleep = tickDurationStopwatchTicks - elapsedTicks;

					if (ticksToSleep > 0)
					{
						// Convert stopwatch ticks to TimeSpan for Thread.Sleep
						double secondsToSleep = (double)ticksToSleep / Stopwatch.Frequency;
						if (secondsToSleep > 0)
						{
		...
		private void processConnectionPayload(ByteArrayReaderWriter reader, NetcodePacketHeader header, int size, EndPoint sender)
		{
			...
			// CRITICAL: Touch the client to prevent timeout when receiving payload packets.
			// Previously, only keepalive packets would Touch() the client, causing connections
			// to timeout even when actively receiving game data. Hot standby connections use
			// payload packets for their keepalives, not protocol keepalives.
		...
		private void processConnectionRequest(ByteArrayReaderWriter reader, int size, EndPoint sender)
		{
			...
			{
				log("Failed to read request for sender: {0}", NetcodeLogLevel.Warning, sender.ToString());
				return;
			}
			...
			{
				log("Connect token expired for sender: {0}", NetcodeLogLevel.Warning, sender.ToString());
			...
			{
				log("Failed to read private token for sender: {0}", NetcodeLogLevel.Warning, sender.ToString());
				connectionRequestPacket.Release();
				return;
			}
		...
		private void disconnectClient(RemoteClient client)
		{
			...
			// Always remove mappings when disconnecting to prevent stale/duplicate entries.
			encryptionManager.RemoveAllEncryptionMappings(client.RemoteEndpoint);
		}
		...
		*/
        ...
        private void sendPayloadToClient(RemoteClient client, byte[] payload, int payloadSize)
		{
            ...
            if (cryptIdx == -1)
            {
				requestDisconnect(client, "MissingEncryptionMapping:PayloadSend");
                return;
            }
		...
		private void sendKeepAlive(RemoteClient client)
		{
			...
			if (cryptIdx == -1)
			{
				requestDisconnect(client, "MissingEncryptionMapping:KeepAliveSend");
				return;
			}
		...
		private void log(string log, NetcodeLogLevel logLevel)
		{
            ...
            switch (logLevel)
            {
                ...
                case NetcodeLogLevel.Info:
                    ...
                    break;
                case NetcodeLogLevel.Warning:
                    GONet.GONetLog.Warning(log);
                    break;
		...
		private void log(string log, NetcodeLogLevel logLevel, params object[] args)
		{
            ...
            switch (logLevel)
            {
                ...
                case NetcodeLogLevel.Info:
                    ...
                    break;
				case NetcodeLogLevel.Warning:
					GONet.GONetLog.Warning(string.Format(log, args));
					break;
                case NetcodeLogLevel.Error:
