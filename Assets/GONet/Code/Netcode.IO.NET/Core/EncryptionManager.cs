...
namespace NetcodeIO.NET
{
	internal class EncryptionManager
	{
        ...
        {
            // Previous implementation used a per-index "shift left" compaction with a HashSet of removed indexes.
            // HashSet iteration order is nondeterministic and multiple removals could corrupt the mapping table.
            // This single-pass compaction is deterministic and self-heals any stray used entries.

            int removedCount = 0;
            int writeIndex = 0;

            for (int readIndex = 0; readIndex < encyrptionMappings_totalCount; readIndex++)
            {
                if (encryptionMappings[readIndex].IsReset)
                {
                    continue;
                }

                if (GONet.Utils.NetworkUtils.AreSameAddressFamilyOrMapped(encryptionMappings[readIndex].Address, address))
                {
                    encryptionMappings[readIndex].Reset();
                    removedCount++;
                    continue;
                }
                ...
                {
                    encryptionMappings[writeIndex].Address = encryptionMappings[readIndex].Address;
                    encryptionMappings[writeIndex].ExpiresAtSeconds = encryptionMappings[readIndex].ExpiresAtSeconds;
                    encryptionMappings[writeIndex].LastAccessedAtSeconds = encryptionMappings[readIndex].LastAccessedAtSeconds;
                    encryptionMappings[writeIndex].TimeoutAfterSeconds = encryptionMappings[readIndex].TimeoutAfterSeconds;
                    encryptionMappings[writeIndex].ClientID = encryptionMappings[readIndex].ClientID;

                    Buffer.BlockCopy(encryptionMappings[readIndex].SendKey, 0, encryptionMappings[writeIndex].SendKey, 0, 32);
                    Buffer.BlockCopy(encryptionMappings[readIndex].ReceiveKey, 0, encryptionMappings[writeIndex].ReceiveKey, 0, 32);
            ...
            // Ensure anything beyond the compacted prefix is reset (and keys cleared).
            for (int i = writeIndex; i < encyrptionMappings_totalCount; i++)
            {
                if (!encryptionMappings[i].IsReset)
                {
                    encryptionMappings[i].Reset();
            ...
            encyrptionMappings_usedCount = writeIndex;
