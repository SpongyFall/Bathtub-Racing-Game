...
namespace GONet
{
    ...
    public partial class RpcEvent : ITransientEvent, ISelfReturnEvent
    {
        ...
        private static void Return(RpcEvent evt)
        {
            ...
            evt.OriginatorAuthorityId = 0;  // CRITICAL: Reset to avoid stale values causing skip logic issues
            evt.HasValidation = false;      // CRITICAL: Reset to avoid stale values affecting validation checks
    ...
    public partial class RoutedRpcEvent : ITransientEvent, ISelfReturnEvent
    {
        ...
        private static void Return(RoutedRpcEvent evt)
        {
            ...
            evt.Data = null;
            evt.CorrelationId = 0;
            evt.OriginatorAuthorityId = 0;
            evt.HasValidation = false;
            evt.ShouldExpandToAllClients = false;
    ...
    public struct GONetRpcContext
    {
        ...
        /// <summary>
        /// The RpcId of the incoming RPC that created this context.
        /// Used to differentiate between the originator calling THIS RPC vs a DIFFERENT RPC.
        /// This prevents incorrect skip-originator behavior in request-response patterns
        /// where ServerRpc calls TargetRpc to respond to the same client.
        /// </summary>
        public readonly uint SourceRpcId;

        public RpcValidationContext ValidationContext { get; internal set; }

        // Constructor from envelope (for real RPC calls)
        internal GONetRpcContext(GONetEventEnvelope envelope, uint sourceRpcId)
        {
            Envelope = envelope;
        ...
        internal GONetRpcContext(ushort sourceAuthorityId, bool isReliable, uint gonetParticipantId)
        {
            ...
            SourceRpcId = 0; // Local calls don't have a source RPC
            ValidationContext = default;
        }
        ...
        internal GONetRpcContext(ushort sourceAuthorityId, bool isReliable, uint gonetParticipantId, bool isSourceRemote)
        {
            ...
            IsReliable = isReliable;
            GONetParticipantId = gonetParticipantId;
            SourceRpcId = 0; // Synthetic context doesn't have a source RPC
            ValidationContext = default;
        }
    }
