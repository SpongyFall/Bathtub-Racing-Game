...
namespace GONet
{
    ...
    public sealed partial class GONetEventBus
    {
        ...
        private static class TypeHierarchyCache
        {
            ...
            /// <summary>
            /// Clears all cached type hierarchy data for session reset (Fast Iteration Mode or lobby flow).
            /// This forces re-discovery of event types on next EnsureInitialized() call.
            /// Called by GONetEventBus.ClearAllSubscriptions().
            /// </summary>
            internal static void ClearAllCaches()
            {
                lock (initLock)
                {
                    baseTypeChainsCache.Clear();
                    allInterfacesCache.Clear();
                    allEventTypes.Clear();
                    isInitialized = false;
                }
                GONetLog.Debug("[TypeHierarchyCache] All caches cleared for session reset.");
            }
        }
        ...
        private sealed class GONetEventHandlerMappings
        {
            ...
            /// <summary>
            /// Clears all event handler subscriptions from this mapping.
            /// Used during session reset (Fast Iteration Mode or lobby flow) to prevent duplicate handlers.
            /// </summary>
            public void ClearAll()
            {
                handlersByEventType_SpecificOnly.Clear();
                handlersByEventType_IncludingChildren.Clear();
                publishTargets_priorityOrderedHandlersByEventType_FULLY_CACHED.Clear();
                dirtyTypes.Clear();
                cacheNeedsRebuild = false;
                eventTypesSpecific?.Clear();
            }
        }
        ...
        {
            // Get the assembly where the generated sync event types are defined.
            // Using SyncEvent_GeneratedTypes as it's in the same assembly as the generated types.
            // NOTE: Type.GetType() without assembly-qualified name only searches the calling assembly
            // and mscorlib, which may fail to find types depending on assembly loading order.
            var assembly = typeof(SyncEvent_GeneratedTypes).Assembly;

            foreach (SyncEvent_GeneratedTypes eventType in Enum.GetValues(typeof(SyncEvent_GeneratedTypes)))
            {
                Type @type = assembly.GetType("GONet." + eventType.ToString());
        ...
        /// <summary>
        /// Re-attempts to resolve any null entries in the event type map.
        /// This is critical for Fast Iteration Mode where the singleton may have been created
        /// before all generated types were available (due to assembly loading order at Editor startup).
        /// Only re-resolves entries that are currently null to avoid unnecessary work.
        /// </summary>
        private void ReinitializeEventTypeMapIfNeeded()
        {
            var assembly = typeof(SyncEvent_GeneratedTypes).Assembly;
            int resolvedCount = 0;
            int nullEntryCount = 0;
            int stillUnresolvedCount = 0;

            foreach (SyncEvent_GeneratedTypes eventType in Enum.GetValues(typeof(SyncEvent_GeneratedTypes)))
            {
                // Only re-attempt resolution if the current entry is null
                if (eventEnumTypeToEventTypeMap.TryGetValue(eventType, out Type existingType) && existingType == null)
                {
                    nullEntryCount++;
                    Type @type = assembly.GetType("GONet." + eventType.ToString());
                    if (@type != null)
            ...
            int totalEntries = eventEnumTypeToEventTypeMap.Count;
            if (nullEntryCount > 0)
            {
                GONetLog.Info($"[GONetEventBus] Fast Iteration Mode: Found {nullEntryCount} null entries out of {totalEntries} total in event type map. Re-resolved {resolvedCount}, still unresolved {stillUnresolvedCount}. Assembly: {assembly.FullName}");
            }
            else
            {
                GONetLog.Debug($"[GONetEventBus] Fast Iteration Mode: No null entries found in event type map ({totalEntries} entries all have valid types).");
            }

            // If there are still unresolved types, try searching all loaded assemblies
            if (stillUnresolvedCount > 0)
            {
                GONetLog.Warning($"[GONetEventBus] {stillUnresolvedCount} types still unresolved from primary assembly. Searching all loaded assemblies...");

                var allAssemblies = AppDomain.CurrentDomain.GetAssemblies();
                int additionalResolved = 0;

                foreach (SyncEvent_GeneratedTypes eventType in Enum.GetValues(typeof(SyncEvent_GeneratedTypes)))
                {
                    if (eventEnumTypeToEventTypeMap.TryGetValue(eventType, out Type existingType) && existingType == null)
                    {
                        string typeName = "GONet." + eventType.ToString();
                        foreach (var asm in allAssemblies)
                        {
                            ...
                            if (@type != null)
                            {
                                eventEnumTypeToEventTypeMap[eventType] = @type;
                                additionalResolved++;
                                GONetLog.Debug($"[GONetEventBus] Found type {typeName} in assembly {asm.GetName().Name}");
                                break;
                            }
                        }
                    }
                ...
                if (additionalResolved > 0)
                {
                    GONetLog.Info($"[GONetEventBus] Fast Iteration Mode: Additionally resolved {additionalResolved} types from other assemblies.");
                }
            }
        }
        ...
        public int Publish<T>(T @event, ushort remoteSourceAuthorityId = default, ushort targetClientAuthorityId = GONetMain.OwnerAuthorityId_Unset, bool shouldPublishReliably = true) where T : IGONetEvent
        {
...
#if UNITY_EDITOR
            // PERFORMANCE METRICS: Track publish timing (EDITOR ONLY)
            // PERF FIX (Dec 2025): Use HighResolutionTimeUtils instead of Stopwatch.StartNew() to avoid 40 byte allocation per call
            long publishStartTicks = 0;
            bool isSyncEvent = @event is SyncEvent_ValueChangeProcessed;
            if (isSyncEvent)
            {
                publishStartTicks = GONet.Utils.HighResolutionTimeUtils.UtcNowTicks;
                _totalPublishCalls++;
            }
#endif
...
#if UNITY_EDITOR
                // PERFORMANCE METRICS: Record elapsed time (EDITOR ONLY)
                // PERF FIX (Dec 2025): Use tick difference instead of Stopwatch to avoid allocation
                if (publishStartTicks > 0)
                {
                    long elapsedTicks = GONet.Utils.HighResolutionTimeUtils.UtcNowTicks - publishStartTicks;
                    _totalPublishTimeMs += elapsedTicks / 10000.0; // ticks to milliseconds (10,000 ticks = 1ms)
                }
#endif
            }
            ...
            int Handle<T>(T @event, ushort remoteSourceAuthorityId, ushort targetClientAuthorityId, bool shouldPublishReliably, int exceptionsThrown, List<EventHandlerAndFilterer> handlersForType) where T : IGONetEvent
            {
                ...
                // COMMENTED (log cleanup) - fires on every RPC event, very spammy
                ...
                if (handlersForType != null)
                {
                    ...
                    for (int i = 0; i < handlerCount; ++i)
                    {
                        ...
                        // COMMENTED (log cleanup) - fires on every RPC event handler, very spammy
                        /*if (isRpcEvent)
                        {
                        ...
                        if (filterPassed)
                        {
                            // PERFORMANCE METRICS: Track handler invocations
#if UNITY_EDITOR
                            if (publishStartTicks > 0) _totalHandlerInvocations++;
#endif
                ...
                {
                    // COMMENTED (log cleanup) - diagnostic code that was using isRpcEvent which is now commented out
                    /*if (isRpcEvent)
                    {
        ...
        /// <summary>
        /// Gets the total count of active event subscriptions across all event types.
        /// Used by diagnostic logging to detect subscription leaks.
        /// Thread-safe when called from main thread (as required by event bus).
        /// </summary>
        public int GetSubscriptionCount()
        {
            int totalCount = 0;

            // Count all subscriptions across all handler mappings
            foreach (var handlers in nonSyncEventHandlerMappings.handlersByEventType_SpecificOnly.Values)
            {
                totalCount += handlers.Count;
            }

            foreach (var handlers in specificSyncEventHandlerMappings.handlersByEventType_SpecificOnly.Values)
            {
                totalCount += handlers.Count;
            }

            foreach (var handlers in anySyncEventHandlerMappings.handlersByEventType_SpecificOnly.Values)
            {
                totalCount += handlers.Count;
            }
        ...
        private long _totalPublishCalls = 0;
        private long _totalHandlerInvocations = 0;
        private double _totalPublishTimeMs = 0;
        private double _lastMetricsResetTime = 0;
        private const double METRICS_RESET_INTERVAL_SECONDS = 5.0;

        /// <summary>
        /// Gets performance metrics for the EventBus (publish calls, handler invocations, avg time).
        /// Resets every 5 seconds to show recent performance.
        /// </summary>
        public string GetPerformanceMetrics()
        {
            double currentTime = UnityEngine.Time.time;
            if (_lastMetricsResetTime == 0) _lastMetricsResetTime = currentTime;

            double elapsed = currentTime - _lastMetricsResetTime;
            if (elapsed >= METRICS_RESET_INTERVAL_SECONDS)
            {
                // Reset metrics
                _totalPublishCalls = 0;
                _totalHandlerInvocations = 0;
                _totalPublishTimeMs = 0;
                _lastMetricsResetTime = currentTime;
                return "Resetting...";
            }

            double avgTimePerPublish = _totalPublishCalls > 0 ? _totalPublishTimeMs / _totalPublishCalls : 0;
            double publishesPerSec = elapsed > 0 ? _totalPublishCalls / elapsed : 0;

            return $"Publishes: {_totalPublishCalls} ({publishesPerSec:F0}/s), " +
                   $"Handlers: {_totalHandlerInvocations}, " +
                   $"AvgTime: {avgTimePerPublish:F3}ms";
        }

        #endregion

        /// <summary>
        /// Clears all event subscriptions from the EventBus.
        /// Call this when resetting a session (Fast Iteration Mode or lobby flow) to prevent duplicate handlers.
        /// After calling this, InitEventSubscriptions() should be called to re-register handlers.
        /// </summary>
        public void ClearAllSubscriptions()
        {
            nonSyncEventHandlerMappings.ClearAll();
            specificSyncEventHandlerMappings.ClearAll();
            anySyncEventHandlerMappings.ClearAll();

            // Clear TypeHierarchyCache to force re-discovery on next use
            TypeHierarchyCache.ClearAllCaches();

            // CRITICAL FIX: Reinitialize the event type map for Fast Iteration Mode.
            // The map may contain null entries if types weren't available when singleton was first created
            // (due to assembly loading order at Editor startup). Re-resolving ensures types are found
            // now that assemblies are fully loaded.
            ReinitializeEventTypeMapIfNeeded();

            // Drain publishASAPQueue to prevent stale events from being processed
            int drainedCount = 0;
            while (publishASAPQueue.TryDequeue(out _))
            {
                drainedCount++;
            }
            if (drainedCount > 0)
            {
                GONetLog.Debug($"[GONetEventBus] Drained {drainedCount} events from publishASAPQueue.");
            }
            ...
            _totalHandlerInvocations = 0;
            _totalPublishTimeMs = 0;
            _lastMetricsResetTime = 0;

            GONetMain.MarkEventSubscriptionsCleared();

            GONetLog.Debug("[GONetEventBus] All subscriptions cleared for session reset.");
        }
