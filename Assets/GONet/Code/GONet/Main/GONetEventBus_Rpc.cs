    ...
    public sealed partial class GONetEventBus
    {
        ...
        private sealed class DeferredRpcInfo : ISelfReturnEvent
        {
            ...
            public void Return()
            {
                if (RpcEvent != null)
                {
                    if (RpcEvent.Data != null)
                    {
                        SerializationUtils.TryReturnByteArray(RpcEvent.Data);
                    }
                    RpcEvent.Return();
                }
        ...
        /// <summary>
        /// Clears all deferred RPC state for session reset (Fast Iteration Mode or lobby flow).
        /// </summary>
        internal static void ResetDeferredRpcStateForNewSession()
        {
            int pendingCount = deferredRpcs.Count;
            if (pendingCount > 0)
            {
                for (int i = deferredRpcs.Count - 1; i >= 0; i--)
                {
                    deferredRpcs[i].Return();
                }
                deferredRpcs.Clear();
            }

            deferredRpcsByGoNetId.Clear();
            totalDeferredRpcs = 0;
            successfulDeferredRpcs = 0;
            timedOutDeferredRpcs = 0;

            if (pendingCount > 0)
            {
                GONetLog.Debug($"[RPC] Cleared {pendingCount} deferred RPC(s) for session reset.");
            }

            Instance.ResetPendingRpcTrackingForNewSession();
        }

        private void ResetPendingRpcTrackingForNewSession()
        {
            int pendingResponsesCleared = 0;
            foreach (var kvp in pendingResponses)
            {
                if (pendingResponses.TryRemove(kvp.Key, out var handler))
                {
                    pendingResponsesCleared++;
                    if (handler is IResponseHandler responseHandler)
                    {
                        responseHandler.Cancel();
                    }
                    else if (handler is DeliveryReportHandler reportHandler)
                    {
            ...
            int pendingDeliveryReportsCleared = 0;
            foreach (var kvp in pendingDeliveryReports)
            {
                if (pendingDeliveryReports.TryRemove(kvp.Key, out var tcs))
                {
                    pendingDeliveryReportsCleared++;
                    tcs.TrySetCanceled();
                }
            }

            if (pendingResponsesCleared > 0 || pendingDeliveryReportsCleared > 0)
            {
                GONetLog.Debug($"[RPC] Cleared {pendingResponsesCleared} pending responses and {pendingDeliveryReportsCleared} pending delivery reports for session reset.");
            }
        }
        ...
        {
            RpcEvent deferredRpcEvent = rpcEvent;
            if (!isPersistent && rpcEvent != null)
            {
                // Clone transient RPCs so deferred processing is not corrupted by pooled return.
                deferredRpcEvent = CloneRpcEventForDeferred(rpcEvent);
            }

            uint targetGoNetId = isPersistent ? persistentRpcEvent.GONetId : deferredRpcEvent.GONetId;

            var deferredInfo = DeferredRpcInfo.Borrow();
            deferredInfo.RpcEvent = deferredRpcEvent;
        ...
        private static RpcEvent CloneRpcEventForDeferred(RpcEvent rpcEvent)
        {
            var clone = RpcEvent.Borrow();
            clone.OccurredAtElapsedTicks = rpcEvent.OccurredAtElapsedTicks;
            clone.RpcId = rpcEvent.RpcId;
            clone.GONetId = rpcEvent.GONetId;
            clone.CorrelationId = rpcEvent.CorrelationId;
            clone.IsSingularRecipientOnly = rpcEvent.IsSingularRecipientOnly;
            clone.OriginatorAuthorityId = rpcEvent.OriginatorAuthorityId;
            clone.HasValidation = rpcEvent.HasValidation;

            if (rpcEvent.Data != null)
            {
                int length = rpcEvent.Data.Length;
                byte[] dataCopy = SerializationUtils.BorrowByteArray(length);
                Buffer.BlockCopy(rpcEvent.Data, 0, dataCopy, 0, length);
                clone.Data = dataCopy;
            }
        ...
        public static void ProcessDeferredRpcs()
        {
            ...
            for (int i = deferredRpcs.Count - 1; i >= 0; i--)
            {
                ...
                if (gnp != null)
                {
                    if (!deferred.IsPersistent && !Instance.rpcHandlers.ContainsKey(deferred.RpcEvent.RpcId))
                    {
                        // Handler not ready yet; keep deferred until registration.
        ...
        /// <summary>
        /// Gets the current count of pending deferred RPCs.
        /// Used by diagnostic logging to detect RPC accumulation.
        /// </summary>
        public static int GetDeferredRpcCount()
        {
            return deferredRpcs.Count;
        }
        ...
        /// <summary>
        /// Walks the inheritance chain to find RPC metadata for a method.
        /// This enables derived classes to call inherited RPC methods without needing to override them.
        /// </summary>
        /// <param name="instanceType">The runtime type of the instance (may be a derived class)</param>
        /// <param name="methodName">The name of the RPC method to find</param>
        /// <param name="metadata">The found RPC metadata, or default if not found</param>
        /// <param name="declaringType">The type where the RPC metadata was found (base class if inherited)</param>
        /// <returns>True if metadata was found, false otherwise</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool TryGetRpcMetadataWithInheritance(Type instanceType, string methodName, out RpcMetadata metadata, out Type declaringType)
        {
            Type currentType = instanceType;
            while (currentType != null && currentType != typeof(object))
            {
                if (rpcMetadata.TryGetValue(currentType, out var typeMetadata) &&
                    typeMetadata.TryGetValue(methodName, out metadata))
                {
        ...
        /// <summary>
        /// Walks the inheritance chain to find an RPC dispatcher for a type.
        /// This enables derived classes to use inherited RPC dispatchers.
        /// </summary>
        /// <param name="instanceType">The runtime type of the instance (may be a derived class)</param>
        /// <param name="dispatcher">The found dispatcher, or null if not found</param>
        /// <returns>True if a dispatcher was found, false otherwise</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool TryGetRpcDispatcherWithInheritance(Type instanceType, out IRpcDispatcher dispatcher)
        {
            Type currentType = instanceType;
            while (currentType != null && currentType != typeof(object))
            {
                if (rpcDispatchers.TryGetValue(currentType, out dispatcher))
                {
                    return true;
                }
                currentType = currentType.BaseType;
        ...
        private readonly ConcurrentDictionary<long, TaskCompletionSource<RpcDeliveryReport>> pendingDeliveryReports = new();
        ...
        private void Client_HandleDeliveryReport(GONetEventEnvelope<RpcDeliveryReportEvent> envelope)
        {
            ...
            if (pendingDeliveryReports.TryRemove(reportEvent.CorrelationId, out var tcs))
            {
        ...
        {
            if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            {
        ...
        private async void HandleRpcForMe(GONetEventEnvelope<RpcEvent> envelope)
        {
            ...
            // COMMENTED (log cleanup) - fires on every RPC received, very spammy
            ...
            // CRITICAL: ClientRpc should ONLY execute on clients, never on the server
            // Track metadata so we can make later decisions (e.g., skip-originator only for TargetRpc)
            bool hasMetadata = false;
            RpcMetadata metadata = default;
                ...
                rpcMetadataByType.TryGetValue(componentType, out var typeMetadata))
            {
                ...
                if (methodName != null && typeMetadata.TryGetValue(methodName, out metadata))
                {
                    hasMetadata = true;
                    ...
                    if (metadata.Type == RpcType.ClientRpc && GONetMain.IsServer)
                    {
                        return;
                    }
                }
            ...
            // CRITICAL: Prevent double-execution for non-validated TargetRPCs ONLY.
            // ServerRpc/ClientRpc should never skip based on originator.
            bool shouldSkip = false;
            if (hasMetadata && metadata.Type == RpcType.TargetRpc)
            {
                bool isOriginator = rpcEvent.OriginatorAuthorityId == GONetMain.MyAuthorityId;
                bool hasNoValidation = !rpcEvent.HasValidation;
                shouldSkip = isOriginator && hasNoValidation;
            }

            // COMMENTED (log cleanup) - fires on every RPC, very spammy
            //GONetLog.Debug($"[TARGETRPC-HANDLER] Double-exec check: shouldSkip={shouldSkip}, OriginatorId={rpcEvent.OriginatorAuthorityId}, MyId={GONetMain.MyAuthorityId}, HasValidation={rpcEvent.HasValidation}");

            if (shouldSkip)
            {
                // COMMENTED (log cleanup) - fires when skipping duplicate RPC
            ...
            // Set context for this RPC execution - include RpcId to track which RPC created this context
            // This is critical for request-response patterns: when a ServerRpc calls a TargetRpc to respond
            // to the same client, we need to know the response is a DIFFERENT RPC than the request
            currentRpcContext = new GONetRpcContext(envelope, rpcEvent.RpcId);
        ...
        private async void HandlePersistentRpcForMe(GONetEventEnvelope<PersistentRpcEvent> envelope)
        {
            ...
            // CRITICAL: ServerRpc should ONLY execute on the server, never on clients
            // CRITICAL: ClientRpc should ONLY execute on clients, never on the server
            // Check if this is a ServerRpc or ClientRpc by looking up its metadata
            if (componentTypeByRpcId.TryGetValue(rpcEvent.RpcId, out var componentType) &&
                rpcMetadataByType.TryGetValue(componentType, out var typeMetadata))
            {
                string methodName = GetMethodNameFromRpcId(rpcEvent.RpcId);
            ...
            // Include RpcId to track which RPC created this context (same fix as HandleRpcForMe)
            currentRpcContext = new GONetRpcContext(transientEnvelope, rpcEvent.RpcId);
        ...
        private async void HandleServerRpc(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            {
                SendRpc0(instance, methodName, metadata.IsReliable, declaringType);
            }
        }

        // HandleServerRpc - 1 parameter
        private async void HandleServerRpc<T1>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            {
                SendRpc1(instance, methodName, metadata.IsReliable, declaringType, arg1);
            }
        }

        // HandleServerRpc - 2 parameters
        private async void HandleServerRpc<T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            {
                SendRpc2(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2);
            }
        }

        // HandleServerRpc - 3 parameters
        private async void HandleServerRpc<T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            {
                SendRpc3(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3);
            }
        }

        // HandleServerRpc - 4 parameters
        private async void HandleServerRpc<T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            else { SendRpc4(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4); }
        }

        // HandleServerRpc - 5 parameters
        private async void HandleServerRpc<T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            else { SendRpc5(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5); }
        }

        // HandleServerRpc - 6 parameters
        private async void HandleServerRpc<T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            else { SendRpc6(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6); }
        }

        // HandleServerRpc - 7 parameters
        private async void HandleServerRpc<T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            else { SendRpc7(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7); }
        }

        // HandleServerRpc - 8 parameters
        private async void HandleServerRpc<T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            if (GONetMain.IsServer)
            {
                if (TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
            ...
            else { SendRpc8(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8); }
        }

        // HandleClientRpc - 0 parameters
        private void HandleClientRpc(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
                if (GONetMain.IsClient && TryGetRpcDispatcherWithInheritance(instance.GetType(), out var dispatcher))
                {
                    var context = new GONetRpcContext(GONetMain.MyAuthorityId, metadata.IsReliable, instance.GONetParticipant.GONetId);
                    SetCurrentRpcContext(context);
                    try
                ...
                // Then broadcast to all clients - use declaring type for RpcId to support inheritance
                var rpcId = GetRpcId(declaringType, methodName);
                ...
                else
                {
                    ...
                    transientEvent.OriginatorAuthorityId = GONetMain.MyAuthorityId; // Server's ID - clients won't skip
        ...
        private void HandleClientRpc<T1>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
        ...
        private void HandleClientRpc<T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
        ...
        private void HandleClientRpc<T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
        ...
        private void HandleClientRpc<T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
        ...
        private void HandleClientRpc<T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
        ...
        private void HandleClientRpc<T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
        ...
        private void HandleClientRpc<T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
        ...
        private void HandleClientRpc<T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            if (GONetMain.IsServer)
            {
                // Execute locally ONLY if server is also a client (listen-server mode)
        ...
        private void HandleTargetRpc(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType)
        {
            ...
            try
            {
                ...
                if (GONetMain.MyAuthorityId != GONetMain.OwnerAuthorityId_Server)
                {
                    if (IsSuitableForPersistence(metadata))
                    {
                        ...
                        persistentRoutedRpc.RpcId = GetRpcId(declaringType, methodName);
                    ...
                    else
                    {
                        ...
                        routedRpc.RpcId = GetRpcId(declaringType, methodName);
                ...
                if (GONetMain.IsServer)
                {
                    ...
                    uint currentRpcId = GetRpcId(declaringType, methodName);

                    // Check RpcContext to see if this came from a client
                    // Also check if this is the SAME RPC the originator called (vs a response RPC)
                    bool isSameRpcAsContext = false;
                    if (currentRpcContext.HasValue && currentRpcContext.Value.SourceAuthorityId != GONetMain.MyAuthorityId)
                    {
                        // Only consider this a "same RPC" if the RpcId matches
                        // This prevents incorrect skip behavior in request-response patterns
                        // where ServerRpc calls TargetRpc to respond to the same client
                        isSameRpcAsContext = currentRpcContext.Value.SourceRpcId == currentRpcId;

                        // CRITICAL: Only inherit the originator's authority ID if this is the SAME RPC they called
                        // For response RPCs (different RpcId), keep originatorAuthorityId as server's ID
                        // This ensures the routing handler doesn't incorrectly skip sending to the original client
                        //
                        // KNOWN LIMITATION: If server receives TargetRpc_X from client and responds with the SAME
                        // TargetRpc_X method back to that client, it will be incorrectly skipped. This is because
                        // we track by RpcId alone and cannot distinguish "echoing same call" vs "new call of same method".
                        // Workaround: Use different RPC methods for request vs response (e.g., RequestSync → SyncResponse).
                        // See: .claude/RpcSkipOriginatorLimitation.md
                        if (isSameRpcAsContext)
                        {
                    ...
                    for (int i = 0; i < validationResult.TargetCount; i++)
                    {
                        if (validationResult.AllowedTargets[i] && targetBuffer[i] != GONetMain.MyAuthorityId)
                        {
                            // Skip sending back to originator ONLY if:
                            // 1. No validation (they already executed locally)
                            // 2. AND this is the SAME RPC method the originator called (not a response to a different RPC)
                            // This fixes the bug where ServerRpc→TargetRpc responses were incorrectly skipped
                            if (!hasValidation && targetBuffer[i] == originatorAuthorityId && isSameRpcAsContext)
                            {
                                continue; // Originator already executed locally - don't echo back
                            }
                            ...
                            {
                                var persistentRpcEvent = new PersistentRpcEvent();
                                persistentRpcEvent.RpcId = GetRpcId(declaringType, methodName);
                            ...
                            {
                                var rpcEvent = RpcEvent.Borrow();
                                rpcEvent.RpcId = GetRpcId(declaringType, methodName);
        ...
        private void HandleTargetRpc<T1>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1)
        {
            ushort[] targetBuffer = targetAuthorityArrayPool.Borrow(MAX_RPC_TARGETS);
            ushort[] deniedBuffer = null;
            int targetCount = 0;
        ...
        private void HandleTargetRpc<T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2)
        {
            ushort[] targetBuffer = targetAuthorityArrayPool.Borrow(MAX_RPC_TARGETS);
            ushort[] deniedBuffer = null;
            int targetCount = 0;
        ...
        private void HandleTargetRpc<T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3)
        {
            ushort[] targetBuffer = targetAuthorityArrayPool.Borrow(MAX_RPC_TARGETS);
            ushort[] deniedBuffer = null;
            int targetCount = 0;
        ...
        private void HandleTargetRpc<T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            ushort[] targetBuffer = targetAuthorityArrayPool.Borrow(MAX_RPC_TARGETS);
            ushort[] deniedBuffer = null;
            int targetCount = 0;
        ...
        private void HandleTargetRpc<T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            ushort[] targetBuffer = targetAuthorityArrayPool.Borrow(MAX_RPC_TARGETS);
            ushort[] deniedBuffer = null;
            int targetCount = 0;
                ...
                if (GONetMain.IsServer)
                {
                    ...
                    for (int i = 0; i < validationResult.TargetCount; i++)
                    {
                        if (validationResult.AllowedTargets[i] && targetBuffer[i] != GONetMain.MyAuthorityId)
                        {
                            ...
                            {
                                var persistentRpcEvent = new PersistentRpcEvent();
                                persistentRpcEvent.RpcId = currentRpcId;
                                persistentRpcEvent.GONetId = instance.GONetParticipant.GONetId;
                                persistentRpcEvent.Data = serializedCopy;
                                persistentRpcEvent.OccurredAtElapsedTicks = GONetMain.Time.ElapsedTicks;
                                persistentRpcEvent.IsSingularRecipientOnly = true;
        ...
        private void HandleTargetRpc<T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            ushort[] targetBuffer = targetAuthorityArrayPool.Borrow(MAX_RPC_TARGETS);
            ushort[] deniedBuffer = null;
            int targetCount = 0;
        ...
        private void HandleTargetRpc<T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            ushort[] targetBuffer = targetAuthorityArrayPool.Borrow(MAX_RPC_TARGETS);
            ushort[] deniedBuffer = null;
            int targetCount = 0;
        ...
        private void HandleTargetRpc<T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            ushort[] targetBuffer = targetAuthorityArrayPool.Borrow(MAX_RPC_TARGETS);
            ushort[] deniedBuffer = null;
            int targetCount = 0;
        ...
        private void SendRpc0(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var rpcEvent = RpcEvent.Borrow();
            rpcEvent.RpcId = rpcId;
        ...
        private void SendRpc1<T1>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var data = new RpcData1<T1> { Arg1 = arg1 };
            int bytesUsed;
        ...
        // SendRpc2
        private void SendRpc2<T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var data = new RpcData2<T1, T2> { Arg1 = arg1, Arg2 = arg2 };
            int bytesUsed;
        ...
        // SendRpc3
        private void SendRpc3<T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var data = new RpcData3<T1, T2, T3> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3 };
            int bytesUsed;
        ...
        // SendRpc4
        private void SendRpc4<T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var data = new RpcData4<T1, T2, T3, T4> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4 };
            int bytesUsed;
        ...
        // SendRpc5
        private void SendRpc5<T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var data = new RpcData5<T1, T2, T3, T4, T5> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4, Arg5 = arg5 };
            int bytesUsed;
        ...
        // SendRpc6
        private void SendRpc6<T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var data = new RpcData6<T1, T2, T3, T4, T5, T6> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4, Arg5 = arg5, Arg6 = arg6 };
            int bytesUsed;
        ...
        // SendRpc7
        private void SendRpc7<T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var data = new RpcData7<T1, T2, T3, T4, T5, T6, T7> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4, Arg5 = arg5, Arg6 = arg6, Arg7 = arg7 };
            int bytesUsed;
        ...
        // SendRpc8
        private void SendRpc8<T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            var data = new RpcData8<T1, T2, T3, T4, T5, T6, T7, T8> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4, Arg5 = arg5, Arg6 = arg6, Arg7 = arg7, Arg8 = arg8 };
            int bytesUsed;
        ...
        {
            if (!TryGetRpcMetadataWithInheritance(instance.GetType(), methodName, out var metadata, out var declaringType))
            {
            ...
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType);
                    break;
            }
        }
        ...
        internal void CallRpcInternal<T1>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1)
        {
            ...
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType, arg1);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType, arg1);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType, arg1);
                    break;
            }
        }
        ...
        internal void CallRpcInternal<T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2)
        {
            ...
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType, arg1, arg2);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType, arg1, arg2);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType, arg1, arg2);
                    break;
            }
        }
        ...
        internal void CallRpcInternal<T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3)
        {
            ...
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3);
                    break;
            }
        }
        ...
        internal void CallRpcInternal<T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            ...
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4);
                    break;
            }
        }
        ...
        internal void CallRpcInternal<T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            ...
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5);
                    break;
            }
        }
        ...
        internal void CallRpcInternal<T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            ...
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6);
                    break;
            }
        }
        ...
        internal void CallRpcInternal<T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            ...
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
                    break;
            }
        }
        ...
        internal void CallRpcInternal<T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            ...
            {
                case RpcType.ServerRpc:
                    HandleServerRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
                    break;
                case RpcType.ClientRpc:
                    HandleClientRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
                    break;
                case RpcType.TargetRpc:
                    HandleTargetRpc(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
                    break;
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private (bool flowControl, TResult value) CallRpcInternalAsync_PreValidation<TResult>(GONetParticipantCompanionBehaviour instance, string methodName, out RpcMetadata metadata, out Type declaringType)
        {
            metadata = default;
            declaringType = null;

            if (!TryGetRpcMetadataWithInheritance(instance.GetType(), methodName, out metadata, out declaringType))
            {
                GONetLog.Warning($"No RPC metadata found for {instance.GetType().Name}.{methodName}");
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult>(GONetParticipantCompanionBehaviour instance, string methodName)
        {
            ...
            Type declaringType;
            (bool flowControl, TResult value) = CallRpcInternalAsync_PreValidation<TResult>(instance, methodName, out metadata, out declaringType);
            if (!flowControl)
            {
                return value;
            }

            return await HandleRpcAsync<TResult>(instance, methodName, metadata, declaringType);
        }
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult, T1>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1)
        {
            ...
            return await HandleRpcAsync<TResult, T1>(instance, methodName, metadata, declaringType, arg1);
        }
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult, T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2)
        {
            ...
            return await HandleRpcAsync<TResult, T1, T2>(instance, methodName, metadata, declaringType, arg1, arg2);
        }
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult, T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3)
        {
            ...
            return await HandleRpcAsync<TResult, T1, T2, T3>(instance, methodName, metadata, declaringType, arg1, arg2, arg3);
        }
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult, T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            ...
            return await HandleRpcAsync<TResult, T1, T2, T3, T4>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4);
        }
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult, T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            ...
            return await HandleRpcAsync<TResult, T1, T2, T3, T4, T5>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5);
        }
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult, T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            ...
            return await HandleRpcAsync<TResult, T1, T2, T3, T4, T5, T6>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6);
        }
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult, T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            ...
            return await HandleRpcAsync<TResult, T1, T2, T3, T4, T5, T6, T7>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
        }
        ...
        internal async Task<TResult> CallRpcInternalAsync<TResult, T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            ...
            return await HandleRpcAsync<TResult, T1, T2, T3, T4, T5, T6, T7, T8>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
        }

        // HandleRpcAsync - 0 parameters
        private async Task<TResult> HandleRpcAsync<TResult>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    else
                    {
                        ...
                        return await SendRpcToDirectRemotesAsync<TResult>(instance, methodName, metadata.IsReliable, declaringType);
                    }
                ...
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        ...
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    if (typeof(TResult) == typeof(RpcDeliveryReport))
                    {
                        ...
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult>(instance, methodName, metadata, declaringType);
                    }
                    else
                    {
        ...
        private async Task<TResult> HandleRpcAsync<TResult, T1>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    {
                        // Client sends ServerRpc to server and waits for response
                        return await SendRpcToDirectRemotesAsync<TResult, T1>(instance, methodName, metadata.IsReliable, declaringType, arg1);
                    }
                ...
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        // Server sends rpc to all clients
                        // This shouldn't return a value typically
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType, arg1);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    ...
                    {
                        // TargetRpc with delivery report goes through special handling
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult, T1>(instance, methodName, metadata, declaringType, arg1);
                    }
                    else
                    {
                        // Regular async TargetRpc (if such a thing exists)
        ...
        /// </summary>
        private async Task<TResult> HandleRpcAsync<TResult, T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    {
                        // Client sends ServerRpc to server and waits for response
                        return await SendRpcToDirectRemotesAsync<TResult, T1, T2>(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2);
                    }
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        // Server sends rpc to all clients
                        // This shouldn't return a value typically
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    ...
                    {
                        // TargetRpc with delivery report goes through special handling
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2>(instance, methodName, metadata, declaringType, arg1, arg2);
                    }
                    else
                    {
                        // Regular async TargetRpc (if such a thing exists)
        ...
        /// Handles asynchronous RPC execution based on the RPC type, executing locally or sending to appropriate targets.
        /// </summary>
        private async Task<TResult> HandleRpcAsync<TResult, T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    {
                        // Client sends ServerRpc to server and waits for response
                        return await SendRpcToDirectRemotesAsync<TResult, T1, T2, T3>(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3);
                    }
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        // Server sends rpc to all clients
                        // This shouldn't return a value typically
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    ...
                    {
                        // TargetRpc with delivery report goes through special handling
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3>(instance, methodName, metadata, declaringType, arg1, arg2, arg3);
                    }
                    else
                    {
                        // Regular async TargetRpc (if such a thing exists)
        ...
        /// Handles asynchronous RPC execution based on the RPC type, executing locally or sending to appropriate targets.
        /// </summary>
        private async Task<TResult> HandleRpcAsync<TResult, T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    {
                        // Client sends ServerRpc to server and waits for response
                        return await SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4>(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4);
                    }
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        // Server sends rpc to all clients
                        // This shouldn't return a value typically
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    ...
                    {
                        // TargetRpc with delivery report goes through special handling
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4);
                    }
                    else
                    {
                        // Regular async TargetRpc (if such a thing exists)
        ...
        /// Handles asynchronous RPC execution based on the RPC type, executing locally or sending to appropriate targets.
        /// </summary>
        private async Task<TResult> HandleRpcAsync<TResult, T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    {
                        // Client sends ServerRpc to server and waits for response
                        return await SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4, T5>(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5);
                    }
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        // Server sends rpc to all clients
                        // This shouldn't return a value typically
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    ...
                    {
                        // TargetRpc with delivery report goes through special handling
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4, T5>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5);
                    }
                    else
                    {
                        // Regular async TargetRpc (if such a thing exists)
        ...
        /// Handles asynchronous RPC execution based on the RPC type, executing locally or sending to appropriate targets.
        /// </summary>
        private async Task<TResult> HandleRpcAsync<TResult, T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    {
                        // Client sends ServerRpc to server and waits for response
                        return await SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4, T5, T6>(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6);
                    }
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        // Server sends rpc to all clients
                        // This shouldn't return a value typically
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    ...
                    {
                        // TargetRpc with delivery report goes through special handling
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4, T5, T6>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6);
                    }
                    else
                    {
                        // Regular async TargetRpc (if such a thing exists)
        ...
        /// Handles asynchronous RPC execution based on the RPC type, executing locally or sending to appropriate targets.
        /// </summary>
        private async Task<TResult> HandleRpcAsync<TResult, T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    {
                        // Client sends ServerRpc to server and waits for response
                        return await SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4, T5, T6, T7>(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
                    }
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        // Server sends rpc to all clients
                        // This shouldn't return a value typically
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    ...
                    {
                        // TargetRpc with delivery report goes through special handling
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4, T5, T6, T7>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
                    }
                    else
                    {
                        // Regular async TargetRpc (if such a thing exists)
        ...
        /// Handles asynchronous RPC execution based on the RPC type, executing locally or sending to appropriate targets.
        /// </summary>
        private async Task<TResult> HandleRpcAsync<TResult, T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            switch (metadata.Type)
            {
                case RpcType.ServerRpc:
                    ...
                    {
                        // Client sends ServerRpc to server and waits for response
                        return await SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4, T5, T6, T7, T8>(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
                    }
                case RpcType.ClientRpc:
                    if (GONetMain.IsServer)
                    {
                        // Server sends rpc to all clients
                        // This shouldn't return a value typically
                        SendRpcToDirectRemotes(instance, methodName, metadata.IsReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
                        return default(TResult);
                    }
                    else
                    {
                ...
                case RpcType.TargetRpc:
                    ...
                    {
                        // TargetRpc with delivery report goes through special handling
                        return await HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4, T5, T6, T7, T8>(instance, methodName, metadata, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
                    }
                    else
                    {
                        // Regular async TargetRpc (if such a thing exists)
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType)
        {
            ...
            try
            {
                ...
                if (GONetMain.IsClient && !GONetMain.IsServer)
                {
                    ...
                    {
                        if (pendingDeliveryReports.TryRemove(correlationId, out var pending))
                        {
                ...
                else if (GONetMain.IsServer)
                {
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult);
                    }
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult, T1>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1)
        {
            // This should only be called for Task<RpcDeliveryReport>
            if (typeof(TResult) != typeof(RpcDeliveryReport))
            {
            ...
            try
            {
                ...
                if (GONetMain.IsClient && !GONetMain.IsServer)
                {
                    ...
                    bool needsReturn;
                    byte[] serialized = SerializationUtils.SerializeToBytes(data, out bytesUsed, out needsReturn);
                    return await SendToServerWithDataDoReporting<TResult>(instance, methodName, metadata, declaringType, targetBuffer, targetCount, correlationId, serialized);
                }
                else if (GONetMain.IsServer)
                {
                    // Check for ASYNC validators first - validate BEFORE serialization with strongly-typed parameters
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        // Handle validation result
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult, arg1);
                    }
                    ...
                    bool needsReturn;
                    byte[] serialized = SerializationUtils.SerializeToBytes(data, out bytesUsed, out needsReturn);
                    return (TResult)(object)Server_SendToValidatedTargetsWithDataDoReporting(instance, methodName, metadata, declaringType, targetBuffer, targetCount, serialized, bytesUsed, needsReturn);
                }
                return default(TResult);
            }
            finally
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2)
        {
            // This should only be called for Task<RpcDeliveryReport>
            if (typeof(TResult) != typeof(RpcDeliveryReport))
            {
            ...
            try
            {
                ...
                else if (GONetMain.IsServer)
                {
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        // Handle validation result
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult, arg1, arg2);
                    }
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3)
        {
            // This should only be called for Task<RpcDeliveryReport>
            if (typeof(TResult) != typeof(RpcDeliveryReport))
            {
            ...
            try
            {
                ...
                else if (GONetMain.IsServer)
                {
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        // Handle validation result
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult, arg1, arg2, arg3);
                    }
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            // This should only be called for Task<RpcDeliveryReport>
            if (typeof(TResult) != typeof(RpcDeliveryReport))
            {
            ...
            try
            {
                ...
                else if (GONetMain.IsServer)
                {
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        // Handle validation result
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult, arg1, arg2, arg3, arg4);
                    }
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4, T5>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            // This should only be called for Task<RpcDeliveryReport>
            if (typeof(TResult) != typeof(RpcDeliveryReport))
            {
            ...
            try
            {
                ...
                else if (GONetMain.IsServer)
                {
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        // Handle validation result
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult, arg1, arg2, arg3, arg4, arg5);
                    }
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4, T5, T6>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            // This should only be called for Task<RpcDeliveryReport>
            if (typeof(TResult) != typeof(RpcDeliveryReport))
            {
            ...
            try
            {
                ...
                else if (GONetMain.IsServer)
                {
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult, arg1, arg2, arg3, arg4, arg5, arg6);
                    }
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4, T5, T6, T7>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            // This should only be called for Task<RpcDeliveryReport>
            if (typeof(TResult) != typeof(RpcDeliveryReport))
            {
            ...
            try
            {
                ...
                else if (GONetMain.IsServer)
                {
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        // Handle validation result and apply any parameter modifications
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
                    }
        ...
        private async Task<TResult> HandleTargetRpcWithDeliveryReportAsync<TResult, T1, T2, T3, T4, T5, T6, T7, T8>(
            GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            // This should only be called for Task<RpcDeliveryReport>
            if (typeof(TResult) != typeof(RpcDeliveryReport))
            {
            ...
            try
            {
                ...
                else if (GONetMain.IsServer)
                {
                        ...
                        asyncValidators.TryGetValue(methodName, out var asyncValidatorMethod))
                    {
                        ...
                        // Handle validation result and apply any parameter modifications
                        return (TResult)(object)await Server_ProcessValidatedTargetsWithDataDoReportingAsync(
                            instance, methodName, metadata, declaringType, targetBuffer, targetCount, validationResult, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
                    }
            ...
            byte[] serialized, int bytesUsed, bool needsReturn)
        {
            ...
            try
            {
                ...
                if (allowedList.Count > 0)
                {
                    ...
                    remoteEvent.RpcId = GetRpcId(declaringType, methodName);
        ...
        private async Task<TResult> SendToServerWithDataDoReporting<TResult>(GONetParticipantCompanionBehaviour instance, string methodName, RpcMetadata metadata, Type declaringType, ushort[] targetBuffer, int targetCount, long correlationId, byte[] data)
        {
            // Client: Send to server for routing and validation
            var tcs = new TaskCompletionSource<RpcDeliveryReport>();
            pendingDeliveryReports[correlationId] = tcs;
        ...
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            ...
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, correlationId);
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult, T1>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            long correlationId = GUID.Generate().AsInt64();
            var tcs = new TaskCompletionSource<TResult>();
            ...
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, arg1, correlationId);
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult, T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            long correlationId = GUID.Generate().AsInt64();
            var tcs = new TaskCompletionSource<TResult>();
            RegisterPendingResponse(correlationId, tcs);
            // Call the void version with correlation ID
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, arg1, arg2, correlationId);
            TResult result = await tcs.Task;
            await GONetThreading.EnsureMainThread();
            return result;
        }
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult, T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            long correlationId = GUID.Generate().AsInt64();
            var tcs = new TaskCompletionSource<TResult>();
            RegisterPendingResponse(correlationId, tcs);
            // Call the void version with correlation ID
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, arg1, arg2, arg3, correlationId);
            TResult result = await tcs.Task;
            await GONetThreading.EnsureMainThread();
            return result;
        }
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            long correlationId = GUID.Generate().AsInt64();
            var tcs = new TaskCompletionSource<TResult>();
            RegisterPendingResponse(correlationId, tcs);
            // Call the void version with correlation ID
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, arg1, arg2, arg3, arg4, correlationId);
            TResult result = await tcs.Task;
            await GONetThreading.EnsureMainThread();
            return result;
        }
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            long correlationId = GUID.Generate().AsInt64();
            var tcs = new TaskCompletionSource<TResult>();
            RegisterPendingResponse(correlationId, tcs);
            // Call the void version with correlation ID
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, arg1, arg2, arg3, arg4, arg5, correlationId);
            TResult result = await tcs.Task;
            await GONetThreading.EnsureMainThread();
            return result;
        }
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            long correlationId = GUID.Generate().AsInt64();
            var tcs = new TaskCompletionSource<TResult>();
            RegisterPendingResponse(correlationId, tcs);
            // Call the void version with correlation ID
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, correlationId);
            TResult result = await tcs.Task;
            await GONetThreading.EnsureMainThread();
            return result;
        }
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            long correlationId = GUID.Generate().AsInt64();
            var tcs = new TaskCompletionSource<TResult>();
            RegisterPendingResponse(correlationId, tcs);
            // Call the void version with correlation ID
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, correlationId);
            TResult result = await tcs.Task;
            await GONetThreading.EnsureMainThread();
            return result;
        }
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private async Task<TResult> SendRpcToDirectRemotesAsync<TResult, T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8)
        {
            var rpcId = GetRpcId(declaringType, methodName);
            long correlationId = GUID.Generate().AsInt64();
            var tcs = new TaskCompletionSource<TResult>();
            RegisterPendingResponse(correlationId, tcs);
            // Call the void version with correlation ID
            SendRpcToDirectRemotes(instance, methodName, isReliable, declaringType, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, correlationId);
            TResult result = await tcs.Task;
            await GONetThreading.EnsureMainThread();
            return result;
        }
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, long correlationId = 0)
        {
            var rpcEvent = RpcEvent.Borrow();
            rpcEvent.RpcId = GetRpcId(declaringType, methodName);
            rpcEvent.GONetId = instance.GONetParticipant.GONetId;
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes<T1>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, long correlationId = 0)
        {
            ...
            SendRpcToDirectRemotesWithData(instance, methodName, isReliable, declaringType, correlationId, serialized);
        }
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes<T1, T2>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, long correlationId = 0)
        {
            // Serialize the arguments
            var data = new RpcData2<T1, T2> { Arg1 = arg1, Arg2 = arg2 };
            int bytesUsed;
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes<T1, T2, T3>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, long correlationId = 0)
        {
            // Serialize the arguments
            var data = new RpcData3<T1, T2, T3> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3 };
            int bytesUsed;
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes<T1, T2, T3, T4>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, long correlationId = 0)
        {
            // Serialize the arguments
            var data = new RpcData4<T1, T2, T3, T4> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4 };
            int bytesUsed;
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes<T1, T2, T3, T4, T5>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, long correlationId = 0)
        {
            // Serialize the arguments
            var data = new RpcData5<T1, T2, T3, T4, T5> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4, Arg5 = arg5 };
            int bytesUsed;
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes<T1, T2, T3, T4, T5, T6>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, long correlationId = 0)
        {
            // Serialize the arguments
            var data = new RpcData6<T1, T2, T3, T4, T5, T6> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4, Arg5 = arg5, Arg6 = arg6 };
            int bytesUsed;
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes<T1, T2, T3, T4, T5, T6, T7>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, long correlationId = 0)
        {
            // Serialize the arguments
            var data = new RpcData7<T1, T2, T3, T4, T5, T6, T7> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4, Arg5 = arg5, Arg6 = arg6, Arg7 = arg7 };
            int bytesUsed;
        ...
        /// on a client, this will send to JUST the server
        /// </summary>
        private void SendRpcToDirectRemotes<T1, T2, T3, T4, T5, T6, T7, T8>(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6, T7 arg7, T8 arg8, long correlationId = 0)
        {
            // Serialize the arguments
            var data = new RpcData8<T1, T2, T3, T4, T5, T6, T7, T8> { Arg1 = arg1, Arg2 = arg2, Arg3 = arg3, Arg4 = arg4, Arg5 = arg5, Arg6 = arg6, Arg7 = arg7, Arg8 = arg8 };
            int bytesUsed;
        ...
        private void SendRpcToDirectRemotesWithData(GONetParticipantCompanionBehaviour instance, string methodName, bool isReliable, Type declaringType, long correlationId, byte[] dataSerialized)
        {
            var rpcEvent = RpcEvent.Borrow();
            rpcEvent.RpcId = GetRpcId(declaringType, methodName);
            rpcEvent.GONetId = instance.GONetParticipant.GONetId;
