...
namespace GONet.Generation
{
	internal sealed class GONetParticipant_AutoMagicalSyncCompanion_Generated_1 : GONetParticipant_AutoMagicalSyncCompanion_Generated
    {
        ...
        internal GONetParticipant_AutoMagicalSyncCompanion_Generated_1(GONetParticipant gonetParticipant) : base(gonetParticipant)
		{
			...
			support5.lastKnownValue_elapsedTicks = shouldSourceFromRigidbody ? GONetMain.Time.FixedElapsedTicks : GONetMain.Time.ElapsedTicks; // Initialize current timestamp
			support5.lastKnownValue_previous_elapsedTicks = shouldSourceFromRigidbody ? GONetMain.Time.FixedElapsedTicks : GONetMain.Time.ElapsedTicks; // Initialize previous timestamp (same as current at initialization)
			...
			support5.syncAttribute_PhysicsUpdateInterval = 2;
			support5.syncAttribute_EnablePhysicsSnapping = false;
			support5.isVelocityEligible = true;
			support5.syncAttribute_VelocityQuantizeLowerBound = -0.133614f;
			support5.syncAttribute_VelocityQuantizeUpperBound = 0.133614f;
			...
			cachedVelocitySerializers[5] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(9, -0.133614f, 0.133614f);
			...
			support6.lastKnownValue_elapsedTicks = shouldSourceFromRigidbody ? GONetMain.Time.FixedElapsedTicks : GONetMain.Time.ElapsedTicks; // Initialize current timestamp
			support6.lastKnownValue_previous_elapsedTicks = shouldSourceFromRigidbody ? GONetMain.Time.FixedElapsedTicks : GONetMain.Time.ElapsedTicks; // Initialize previous timestamp (same as current at initialization)
			...
			support6.syncAttribute_PhysicsUpdateInterval = 2;
			support6.syncAttribute_EnablePhysicsSnapping = false;
			support6.isVelocityEligible = true;
			support6.syncAttribute_VelocityQuantizeLowerBound = -0.366233f;
			support6.syncAttribute_VelocityQuantizeUpperBound = 0.366233f;
			...
			support6.syncAttribute_QuantizerSettingsGroup = new GONet.Utils.QuantizerSettingsGroup(-125f, 125f, 14, true);

			cachedCustomSerializers[6] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(14, -125f, 125f);
			cachedValueSerializers[6] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(14, -125f, 125f);
			cachedVelocitySerializers[6] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(14, -0.366233f, 0.366233f);
        ...
        internal override void SerializeSingle(Utils.BitByBitByteArrayBuilder bitStream_appendTo, byte singleIndex, bool isVelocityBundle = false)
        {
			switch (singleIndex)
			{
				...
				{ // Transform.rotation
					    IGONetAutoMagicalSync_CustomSerializer customSerializer = cachedCustomSerializers[5];
					// VelocitySyncTelemetry.TraceServerAuthority(gonetParticipant.GONetId, 5, "rotation", Transform.rotation);
					if (isVelocityBundle)
					{
						...
						// VelocitySyncTelemetry.TraceServerSend(gonetParticipant.GONetId, 5, "VELOCITY", Transform.rotation, velocityValue.UnityEngine_Vector3, "partition_decision", valuesChangesSupport[5].mostRecentChanges_usedSize, true);
					}
					else
					{
						...
						// VelocitySyncTelemetry.TraceServerSend(gonetParticipant.GONetId, 5, "VALUE", Transform.rotation, UnityEngine.Vector3.zero, "partition_decision", valuesChangesSupport[5].mostRecentChanges_usedSize, false);
					}
				}
				break;
        ...
        internal override void DeserializeInitAll(Utils.BitByBitByteArrayBuilder bitStream_readFrom, long assumedElapsedTicksAtChange)
        {
			...
			{ // Transform.rotation
				...
				valuesChangesSupport[5].AddToMostRecentChangeQueue_IfAppropriate(GONetMain.Time.ElapsedTicks, initSyncableValue);
			}
			{ // Transform.position
				IGONetAutoMagicalSync_CustomSerializer customSerializer = cachedCustomSerializers[6];
				...
				valuesChangesSupport[6].AddToMostRecentChangeQueue_IfAppropriate(GONetMain.Time.ElapsedTicks, initSyncableValue);
			}
        }
			...
			switch (singleIndex)
			{
				...
				case 5:
					if (gonetParticipant.v2_isRegisteredInSoA)
					{
						...
						valuesChangesSupport[5].AddToMostRecentChangeQueue_IfAppropriate(assumedElapsedTicksAtChange, value);
					}
					else
					{
				...
				case 6:
					if (gonetParticipant.v2_isRegisteredInSoA)
					{
						...
						valuesChangesSupport[6].AddToMostRecentChangeQueue_IfAppropriate(assumedElapsedTicksAtChange, value);
					}
					else
					{
						// v1 fallback: Event bus blending
		...
		internal override void UpdateLastKnownValues(GONetMain.SyncBundleUniqueGrouping onlyMatchIfUniqueGroupingMatches)
		{
					...
					!ShouldSkipSync(valuesChangesSupport5, 5)) // TODO examine eval order and performance...should this be first or last?, TODO also consider taking this check out of this condition alltogether, because it is perhaps more expensive to do this check than it is to just execute the body AND the body execution will not actually affect whether or not this value change will get sync'd or not..hmm...
				{
					...
					valuesChangesSupport5.lastKnownValue_previous_elapsedTicks = valuesChangesSupport5.lastKnownValue_elapsedTicks;
					...
					valuesChangesSupport5.lastKnownValue_elapsedTicks = shouldSourceFromRigidbody ? GONetMain.Time.FixedElapsedTicks : GONetMain.Time.ElapsedTicks;
				}
				...
				{
					// Velocity-eligible: Save OLD current to previous BEFORE updating current
					valuesChangesSupport6.lastKnownValue_previous_elapsedTicks = valuesChangesSupport6.lastKnownValue_elapsedTicks;
					...
					valuesChangesSupport6.lastKnownValue_elapsedTicks = shouldSourceFromRigidbody ? GONetMain.Time.FixedElapsedTicks : GONetMain.Time.ElapsedTicks;
				}
