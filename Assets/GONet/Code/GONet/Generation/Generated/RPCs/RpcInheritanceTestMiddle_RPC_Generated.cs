...
{
    internal static class RpcInheritanceTestMiddle_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class RpcInheritanceTestMiddle_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            RpcInheritanceTestMiddle_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class RpcInheritanceTestMiddle_RpcDispatcher : IRpcDispatcher
    {
        ...
        static RpcInheritanceTestMiddle_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(RpcInheritanceTestMiddle));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(RpcInheritanceTestMiddle), new RpcInheritanceTestMiddle_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(RpcInheritanceTestMiddle), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for RpcInheritanceTestMiddle");
            }
        }
