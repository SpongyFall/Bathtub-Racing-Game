...
{
    internal static class RpcInheritanceTestDerived_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class RpcInheritanceTestDerived_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            RpcInheritanceTestDerived_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class RpcInheritanceTestDerived_RpcDispatcher : IRpcDispatcher
    {
        ...
        static RpcInheritanceTestDerived_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(RpcInheritanceTestDerived));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(RpcInheritanceTestDerived), new RpcInheritanceTestDerived_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(RpcInheritanceTestDerived), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for RpcInheritanceTestDerived");
            }
        }
