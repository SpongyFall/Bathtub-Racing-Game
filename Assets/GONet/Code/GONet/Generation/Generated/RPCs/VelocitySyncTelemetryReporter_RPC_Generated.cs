...
namespace GONet
{
    internal static class VelocitySyncTelemetryReporter_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class VelocitySyncTelemetryReporter_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            VelocitySyncTelemetryReporter_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class VelocitySyncTelemetryReporter_RpcDispatcher : IRpcDispatcher
    {
        ...
        static VelocitySyncTelemetryReporter_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(VelocitySyncTelemetryReporter));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(VelocitySyncTelemetryReporter), new VelocitySyncTelemetryReporter_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(VelocitySyncTelemetryReporter), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for VelocitySyncTelemetryReporter");
            }
        }
