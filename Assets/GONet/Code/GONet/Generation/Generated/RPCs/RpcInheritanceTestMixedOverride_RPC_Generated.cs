...
{
    internal static class RpcInheritanceTestMixedOverride_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;

        // Derived class RPC IDs
        internal const uint BaseClass_ServerRpc_UseItem_RpcId = 0x66071C3D;
    }

    internal static class RpcInheritanceTestMixedOverride_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            RpcInheritanceTestMixedOverride_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class RpcInheritanceTestMixedOverride_RpcDispatcher : IRpcDispatcher
    {
        private static bool isInitialized = false;

        private const string BASECLASS_SERVERRPC_USEITEM = nameof(RpcInheritanceTestMixedOverride.BaseClass_ServerRpc_UseItem);
        ...
        public static readonly Dictionary<string, RpcMetadata> Metadata = new Dictionary<string, RpcMetadata>
        {
            ...
            // Derived class RPCs
            { nameof(GONet.Sample.RpcTests.RpcInheritanceTestMixedOverride.BaseClass_ServerRpc_UseItem), new RpcMetadata { 
                Type = RpcType.ServerRpc, 
                IsReliable = true, 
                IsMineRequired = true, 
                IsPersistent = false, 
        ...
        {
            var typed = (GONet.Sample.RpcTests.RpcInheritanceTestMixedOverride)instance;
            switch (methodName)
            {
        ...
        static RpcInheritanceTestMixedOverride_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(RpcInheritanceTestMixedOverride));
                GONetMain.EventBus.RegisterRpcIdMapping(0x66071C3D, nameof(RpcInheritanceTestMixedOverride.BaseClass_ServerRpc_UseItem), typeof(RpcInheritanceTestMixedOverride));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(RpcInheritanceTestMixedOverride), new RpcInheritanceTestMixedOverride_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(RpcInheritanceTestMixedOverride), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for RpcInheritanceTestMixedOverride");
            }
        }
        ...
        private static void RegisterRpcHandlers()
        {
            ...
            eventBus.RegisterRpcHandler(RpcInheritanceTestMixedOverride_RpcIds.BaseClass_ServerRpc_UseItem_RpcId, async (envelope) =>
            {
                var rpcEvent = envelope.Event;
                var gnp = GONetMain.GetGONetParticipantById(rpcEvent.GONetId);
                var instance = gnp?.GetComponent<RpcInheritanceTestMixedOverride>();
                ...
                var args = SerializationUtils.DeserializeFromBytes<RpcInheritanceTestMixedOverride_BaseClass_ServerRpc_UseItem_RpcData>(rpcEvent.Data);
    ...
    [MemoryPackable]
    public partial struct RpcInheritanceTestMixedOverride_BaseClass_ServerRpc_UseItem_RpcData
    {
