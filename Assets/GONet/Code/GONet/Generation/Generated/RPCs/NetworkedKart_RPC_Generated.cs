...
namespace GONet
{
    internal static class NetworkedKart_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;

        // Derived class RPC IDs
        internal const uint RPCSetup_RpcId = 0xAA9B3F58;
    }

    internal static class NetworkedKart_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            NetworkedKart_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class NetworkedKart_RpcDispatcher : IRpcDispatcher
    {
        private static bool isInitialized = false;

        private const string RPCSETUP = nameof(NetworkedKart.RPCSetup);
        ...
        public static readonly Dictionary<string, RpcMetadata> Metadata = new Dictionary<string, RpcMetadata>
        {
            ...
            // Derived class RPCs
            { nameof(NetworkedKart.RPCSetup), new RpcMetadata { 
        ...
        {
            var typed = (NetworkedKart)instance;
            switch (methodName)
            {
                case RPCSETUP:
                    typed.RPCSetup((System.UInt64)(object)arg1);
                    break;
            }
        }
        ...
        static NetworkedKart_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(NetworkedKart));
                GONetMain.EventBus.RegisterRpcIdMapping(0xAA9B3F58, nameof(NetworkedKart.RPCSetup), typeof(NetworkedKart));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(NetworkedKart), new NetworkedKart_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(NetworkedKart), Metadata);
                if (TargetPropertyAccessors.Count > 0)
                    GONetMain.EventBus.RegisterTargetPropertyAccessors(typeof(NetworkedKart), TargetPropertyAccessors);
                if (MultiTargetPropertyAccessors.Count > 0)
                    GONetMain.EventBus.RegisterMultiTargetPropertyAccessors(typeof(NetworkedKart), MultiTargetPropertyAccessors);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for NetworkedKart");
            }
        }
        ...
        private static void RegisterRpcHandlers()
        {
            ...
            eventBus.RegisterRpcHandler(NetworkedKart_RpcIds.RPCSetup_RpcId, async (envelope) =>
            {
                var rpcEvent = envelope.Event;
                var gnp = GONetMain.GetGONetParticipantById(rpcEvent.GONetId);
                var instance = gnp?.GetComponent<NetworkedKart>();
                ...
                var args = SerializationUtils.DeserializeFromBytes<NetworkedKart_RPCSetup_RpcData>(rpcEvent.Data);
                instance.RPCSetup(args.ownerId);
            });
        }
    }

    [MemoryPackable]
    public partial struct NetworkedKart_RPCSetup_RpcData
    {
        public System.UInt64 ownerId { get; set; }
    }
