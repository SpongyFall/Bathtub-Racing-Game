...
{
    internal static class RpcTestRunnerUI_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class RpcTestRunnerUI_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            RpcTestRunnerUI_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class RpcTestRunnerUI_RpcDispatcher : IRpcDispatcher
    {
        ...
        static RpcTestRunnerUI_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(RpcTestRunnerUI));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(RpcTestRunnerUI), new RpcTestRunnerUI_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(RpcTestRunnerUI), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for RpcTestRunnerUI");
            }
        }
