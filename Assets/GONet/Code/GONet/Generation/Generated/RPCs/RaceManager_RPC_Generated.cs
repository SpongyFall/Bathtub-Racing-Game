...
namespace GONet
{
    internal static class RaceManager_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;

        // Derived class RPC IDs
        internal const uint StartRace_RpcId = 0xE7A67714;
    }

    internal static class RaceManager_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            RaceManager_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class RaceManager_RpcDispatcher : IRpcDispatcher
    {
        private static bool isInitialized = false;

        private const string STARTRACE = nameof(RaceManager.StartRace);
        ...
        public static readonly Dictionary<string, RpcMetadata> Metadata = new Dictionary<string, RpcMetadata>
        {
            ...
            // Derived class RPCs
            { nameof(RaceManager.StartRace), new RpcMetadata { 
        ...
        {
            var typed = (RaceManager)instance;
            switch (methodName)
            {
                case STARTRACE:
                    typed.StartRace();
        ...
        static RaceManager_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(RaceManager));
                GONetMain.EventBus.RegisterRpcIdMapping(0xE7A67714, nameof(RaceManager.StartRace), typeof(RaceManager));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(RaceManager), new RaceManager_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(RaceManager), Metadata);
                if (TargetPropertyAccessors.Count > 0)
                    GONetMain.EventBus.RegisterTargetPropertyAccessors(typeof(RaceManager), TargetPropertyAccessors);
                if (MultiTargetPropertyAccessors.Count > 0)
                    GONetMain.EventBus.RegisterMultiTargetPropertyAccessors(typeof(RaceManager), MultiTargetPropertyAccessors);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for RaceManager");
            }
        }
        ...
        private static void RegisterRpcHandlers()
        {
            ...
            eventBus.RegisterRpcHandler(RaceManager_RpcIds.StartRace_RpcId, async (envelope) =>
            {
                var rpcEvent = envelope.Event;
                var gnp = GONetMain.GetGONetParticipantById(rpcEvent.GONetId);
                var instance = gnp?.GetComponent<RaceManager>();
