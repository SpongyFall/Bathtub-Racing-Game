...
{
    internal static class RpcInheritanceTestGrandchild_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class RpcInheritanceTestGrandchild_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            RpcInheritanceTestGrandchild_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class RpcInheritanceTestGrandchild_RpcDispatcher : IRpcDispatcher
    {
        ...
        static RpcInheritanceTestGrandchild_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(RpcInheritanceTestGrandchild));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(RpcInheritanceTestGrandchild), new RpcInheritanceTestGrandchild_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(RpcInheritanceTestGrandchild), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for RpcInheritanceTestGrandchild");
            }
        }
