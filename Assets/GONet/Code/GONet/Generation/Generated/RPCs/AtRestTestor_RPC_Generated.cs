...
namespace GONet
{
    internal static class AtRestTestor_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class AtRestTestor_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            AtRestTestor_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class AtRestTestor_RpcDispatcher : IRpcDispatcher
    {
        ...
        static AtRestTestor_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(AtRestTestor));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(AtRestTestor), new AtRestTestor_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(AtRestTestor), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for AtRestTestor");
            }
        }
