...
namespace GONet
{
    ...
    internal class GONetSampleChatSystem_RpcDispatcher : IRpcDispatcher
    {
        ...
        {
            { nameof(GONetSampleChatSystem.SendMessage), 
              typeof(GONetSampleChatSystem).GetMethod(nameof(GONetSampleChatSystem.ValidateMessageAsync), 
        ...
        {
            { nameof(GONetSampleChatSystem.SendMessage), (data) =>
            {
                var deserialized = SerializationUtils.DeserializeFromBytes<GONetSampleChatSystem_SendMessage_RpcData>(data);
                return new object[] { deserialized.content, deserialized.channelName, deserialized.messageType, deserialized.fromUserId, deserialized.recipients };
            }},
        };
        ...
        {
            { nameof(GONetSampleChatSystem.SendMessage), (args) =>
            {
                var data = new GONetSampleChatSystem_SendMessage_RpcData
                {
                    content = (System.String)args[0],
                    channelName = (System.String)args[1],
                    messageType = (GONetSampleChatSystem.ChatType)args[2],
                    fromUserId = (System.UInt16)args[3],
                    recipients = (System.UInt16[])args[4],
                };
                int bytesUsed;
                bool needsReturn;
        ...
        public static readonly Dictionary<string, RpcMetadata> Metadata = new Dictionary<string, RpcMetadata>
        {
            ...
            { nameof(GONetSampleChatSystem.SendMessage), new RpcMetadata { 
                ...
                ValidationMethodName = "ValidateMessageAsync",
        ...
        static GONetSampleChatSystem_RpcDispatcher()
        {
            if (!isInitialized)
            {
                ...
                if (AsyncValidators != null && AsyncValidators.Count > 0)
                    GONetMain.EventBus.RegisterAsyncValidators(typeof(GONetSampleChatSystem), AsyncValidators, ValidatorParameterCounts);
                if (RpcDeserializers != null && RpcDeserializers.Count > 0)
                    GONetMain.EventBus.RegisterRpcDeserializers(typeof(GONetSampleChatSystem), RpcDeserializers);
                if (RpcSerializers != null && RpcSerializers.Count > 0)
                    GONetMain.EventBus.RegisterRpcSerializers(typeof(GONetSampleChatSystem), RpcSerializers);
                RegisterRpcHandlers();
        ...
        private static void RegisterRpcHandlers()
        {
            ...
            eventBus.RegisterRpcHandler(GONetSampleChatSystem_RpcIds.BroadcastParticipantUpdate_RpcId, async (envelope) =>
            {
                ...
                var instance = gnp?.GetComponent<GONetSampleChatSystem>();
