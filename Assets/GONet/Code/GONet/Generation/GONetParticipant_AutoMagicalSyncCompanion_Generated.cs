...
namespace GONet.Generation
{
    ...
    public abstract class GONetParticipant_AutoMagicalSyncCompanion_Generated : IDisposable
    {
        ...
        /// <summary>
        /// Resets all lastKnownValueAtRestBits to ALREADY_BROADCASTED state during ownership migration.
        /// This prevents the "At Rest but not broadcasted" warning spam after failover when the new
        /// authority starts tracking values that were previously in NEEDS_TO_BROADCAST state.
        /// Called by <see cref="GONetMain.ResetBlendBuffersForOwnershipMigration"/>.
        /// </summary>
        internal void ResetValueAtRestBitsForOwnershipMigration()
        {
            for (int i = 0; i < valuesCount; ++i)
            {
        ...
        internal bool HaveAnyValuesChangedSinceLastCheck_AppendNewlyAtRest(GONetMain.SyncBundleUniqueGrouping onlyMatchIfUniqueGroupingMatches, long nowElapsedTicks, List<GONetMain.AutoMagicalSync_ValueMonitoringSupport_ChangedValue> valuesAtRestToBroadcast)
        {
            ...
            {
                return false; // Early return - nothing can be processed
            }

            // IsOKToStartAutoMagicalProcessing is TRUE, proceed with processing
            {
                for (int i = 0; i < valuesCount; ++i)
                {
                    GONetMain.AutoMagicalSync_ValueMonitoringSupport_ChangedValue valueChangeSupport = valuesChangesSupport[i];

                    bool doesMatch = DoesMatchUniqueGrouping(valueChangeSupport, onlyMatchIfUniqueGroupingMatches);
                    // PERFORMANCE OPTIMIZATION: Short-circuit ShouldSkipSync when grouping doesn't match.
                    // This eliminates ~11,000+ delegate calls per physics frame for animator parameters,
                    // which don't match the physics sync grouping but were still being checked.
                    if (!doesMatch)
                    {
                        continue; // Skip early - no need to call ShouldSkipSync for non-matching values
                    }

                    bool shouldSkip = ShouldSkipSync(valueChangeSupport, i);
                    ...
                    if (!shouldSkip)
                    {
                        if (valueChangeSupport.lastKnownValue == valueChangeSupport.lastKnownValue_previous)
                        {
                            ...
                            { // if the value is the same and our at rest stuff applies here, we need to check if this is (newly) considered 'at rest' or not and act accordingly (i.e., signal that further action needs to be taken to tell others)
                                ...
                                {
                                    // STAGE 1 FIX (Oct 2025): For velocity-eligible values, verify velocity is truly zero before sending at-rest
                                    // Problem: Slow motion (sub-quantization) causes quantized values to appear "same" but object is actually moving
                                    // Solution: Calculate actual velocity and only send at-rest if velocity is effectively zero
                                    bool shouldSendAtRest = true;

                                    if (valueChangeSupport.isVelocityEligible)
                                    {
                                        // Calculate velocity to check if truly at rest
                        ...
                        else
                        {
                            ...
                            if (!AreEqualConsideringQuantization(valueChangeSupport, valueChangeSupport.lastKnownValue, valueChangeSupport.lastKnownValue_previous))
                            {
                                if (lastKnownValueAtRestBits[i] == LAST_KNOWN_VALUE_IS_AT_REST_NEEDS_TO_BROADCAST)
                                {
                                    // NOTE: Commented out - fires frequently during physics-heavy scenarios and is not actionable
        ...
        internal void OnValueChangeCheck_Reset(GONetMain.SyncBundleUniqueGrouping onlyMatchIfUniqueGroupingMatches)
        {
            for (int i = 0; i < valuesCount; ++i)
            {
                if (DoesMatchUniqueGrouping(valuesChangesSupport[i], onlyMatchIfUniqueGroupingMatches))
                {
                    ...
                    // FIX (Oct 2025): Update previous values ONLY for NON-velocity-eligible fields
                    // - Velocity-eligible fields: UpdateLastKnownValues() already updates previous correctly BEFORE reading new value
                    //   Overwriting here would BREAK velocity calculation by making previous == current!
                    // - Non-velocity fields: UpdateLastKnownValues() never touches previous for these fields
                    //   Without this update, lastKnownValue_previous stays at initial value (often 0),
                    //   causing false change detection every frame (e.g., "0→3071" repeatedly for GONetId)
                    if (!valuesChangesSupport[i].isVelocityEligible)
                    {
                        valuesChangesSupport[i].lastKnownValue_previous = valuesChangesSupport[i].lastKnownValue;
                        valuesChangesSupport[i].lastKnownValue_previous_elapsedTicks = valuesChangesSupport[i].lastKnownValue_elapsedTicks;
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Determines if calculated velocity is effectively zero (object truly at rest).
        /// Used by at-rest detection to avoid spamming messages during slow sub-quantization motion.
        ///
        /// Thresholds:
        /// - Linear velocity: 0.001 units/sec (~0.04mm at 24Hz sync rate)
        /// - Angular velocity: 0.01 rad/sec (~0.57 degrees/sec)
        ///
        /// Rationale: These thresholds are well below typical quantization steps (76mm for position),
        /// so any motion above these thresholds will be handled by velocity synthesis.
        /// </summary>
        private bool IsVelocityEffectivelyZero(GONetSyncableValue velocity, GONetMain.AutoMagicalSync_ValueMonitoringSupport_ChangedValue valueChangeSupport)
        {
            const float LINEAR_VELOCITY_THRESHOLD = 0.001f; // 0.001 units/sec
            const float ANGULAR_VELOCITY_THRESHOLD = 0.01f; // 0.01 radians/sec

            if (velocity == null)
            {
                return true; // No velocity data = at rest
            }

            if (velocity.GONetSyncType == GONetSyncableValueTypes.System_Single)
            {
                float vel = velocity.System_Single;
                return Mathf.Abs(vel) < LINEAR_VELOCITY_THRESHOLD;
            }
            else if (velocity.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Vector2)
            {
                UnityEngine.Vector2 vel = velocity.UnityEngine_Vector2;
                return vel.sqrMagnitude < (LINEAR_VELOCITY_THRESHOLD * LINEAR_VELOCITY_THRESHOLD);
            }
            else if (velocity.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Vector3)
            {
                UnityEngine.Vector3 vel = velocity.UnityEngine_Vector3;
                return vel.sqrMagnitude < (LINEAR_VELOCITY_THRESHOLD * LINEAR_VELOCITY_THRESHOLD);
            }
            else if (velocity.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Vector4)
            {
                UnityEngine.Vector4 vel = velocity.UnityEngine_Vector4;
                return vel.sqrMagnitude < (LINEAR_VELOCITY_THRESHOLD * LINEAR_VELOCITY_THRESHOLD);
            }
            else if (velocity.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Quaternion)
            {
                // Quaternion velocity is stored as Vector3 (angular velocity)
                // This shouldn't happen (CalculateVelocity returns Vector3 for Quaternions)
                return true; // Treat as at rest if unexpected type
            }

            return true; // Unknown type, assume at rest
        }
        ...
        public float CalculateDistanceBetweenValues(GONetSyncableValue valueA, GONetSyncableValue valueB)
        {
            ...
            {
                return Mathf.Abs(valueA.System_Single - valueB.System_Single);
            }
            else if (valueA.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Vector2)
            {
                return UnityEngine.Vector2.Distance(valueA.UnityEngine_Vector2, valueB.UnityEngine_Vector2);
            }
            else if (valueA.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Vector3)
            {
                return UnityEngine.Vector3.Distance(valueA.UnityEngine_Vector3, valueB.UnityEngine_Vector3);
            }
            else if (valueA.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Vector4)
            {
                return UnityEngine.Vector4.Distance(valueA.UnityEngine_Vector4, valueB.UnityEngine_Vector4);
            }
            else if (valueA.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Quaternion)
            {
                // Quaternion distance = angle in degrees
                return UnityEngine.Quaternion.Angle(valueA.UnityEngine_Quaternion, valueB.UnityEngine_Quaternion);
            }

            return 0f; // Unknown type
        }

        /// <summary>
        /// Gets the quantization step size for a value.
        /// Used as threshold for smart at-rest value selection.
        ///
        ...
        public float GetQuantizationStepForValue(byte singleIndex)
        {
            ...
            {
                return float.MaxValue; // No quantization = infinite precision
            }

            // Calculate quantization step from settings
            float boundRange = settings.upperBound - settings.lowerBound;
            float maxQuantizedValue = (float)System.Math.Pow(2.0, settings.quantizeToBitCount) - 1f;
            float step = boundRange / maxQuantizedValue;

            // Special handling for Quaternions (Smallest3 encoding)
            // Quaternion serializers have different quantization characteristics
            IGONetAutoMagicalSync_CustomSerializer serializer = cachedCustomSerializers[singleIndex];
            if (serializer is QuaternionSerializer)
            {
                ...
                step = SMALLEST3_RANGE / maxQuantizedValue;
            }
        ...
        /// </summary>
        /// <param name="isAnchor">True for VALUE bundles (anchors) - triggers double-write in SoA to prevent velocity spikes</param>
        internal abstract void InitSingle(GONetSyncableValue value, byte singleIndex, long assumedElapsedTicksAtChange, bool isAnchor = false);
        ...
        internal static bool DoesMatchUniqueGrouping(GONetMain.AutoMagicalSync_ValueMonitoringSupport_ChangedValue valueChangeSupport, GONetMain.SyncBundleUniqueGrouping onlyMatchIfUniqueGroupingMatches)
        {
            ...
            // bypasses frequency matching for physics-related values. This is because:
            ...
            //
            // IMPORTANT: Animator parameters are EXCLUDED from this special case because:
            // - They have their own specific sync frequencies (e.g., 0.05f for floats, 0f ASAP for int/bool)
            // - Matching them to the physics sync group would cause them to be processed by the wrong scheduler
            // - This led to a bug where float animator params (0.05f frequency) were matched by the 0/Unreliable
            //   physics scheduler, causing them to never be picked up by their correct 0.05/Unreliable scheduler
            ...
            bool isAnimatorParameter = !string.IsNullOrEmpty(valueChangeSupport.animatorParameterName);

            if (isPhysicsSyncGrouping && !isAnimatorParameter)
            {
                // Physics sync: Process all non-animator members (frequency check bypassed)
                return onlyMatchIfUniqueGroupingMatches.reliability == valueChangeSupport.syncAttribute_Reliability;
            }

            // PHYSICS OBJECT POSITION/ROTATION EXCLUSION from regular sync pipelines:
            // For physics objects that I OWN (IsRigidBodyOwnerOnlyControlled=true && IsMine), position and rotation
            // are synced via the physics pipeline (0/Unreliable with mustRunOnUnityMainThread=true), not regular schedulers.
            // This prevents double-syncing where position/rotation would match both the physics pipeline
            // (by reliability only) AND regular schedulers (by frequency match like 0.04166667/Unreliable).
            //
            // IMPORTANT: We check IsMine because:
            // - Only the authority (owner) sends sync data for a participant
            // - Physics pipeline only processes physics objects I OWN
            // - If I don't own it, I'm not sending data anyway, so exclusion doesn't apply
            // - This ensures client-owned physics objects work correctly on server and vice versa
            //
            // Animator params on physics objects still need to go through regular schedulers.
            if (!isPhysicsSyncGrouping && !isAnimatorParameter)
            {
                bool isPositionOrRotation = valueChangeSupport.memberName == "position" || valueChangeSupport.memberName == "rotation";
                if (isPositionOrRotation)
                {
                    var participant = valueChangeSupport.syncCompanion?.gonetParticipant;
                    if (participant != null &&
                        participant.IsMine &&  // Only exclude if I'm the authority sending this data
                        participant.IsRigidBodyOwnerOnlyControlled &&
                        participant.myRigidBody != null)
                    {
                        // This is position/rotation on MY physics object - skip in regular sync, handled by physics pipeline
                        return false;
                    }
                }
            }

            // Regular sync (including animator parameters): Match both frequency and reliability
            bool frequencyMatch = onlyMatchIfUniqueGroupingMatches.scheduleFrequency == valueChangeSupport.syncAttribute_SyncChangesEverySeconds;
            bool reliabilityMatch = onlyMatchIfUniqueGroupingMatches.reliability == valueChangeSupport.syncAttribute_Reliability;

            return frequencyMatch && reliabilityMatch;
        }
            ...
            out bool didExtrapolatePastMostRecentChanges)
        {
            ...
            if (hasVelocityData && isVelocityRecent)
            {
                ...
                if (customVelocityBlending != null)
                {
                    ...
                    if (blendedValue.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Quaternion)
                    {
                        VelocitySyncTelemetry.TraceClientBlend(gonetParticipant.GONetId, index, "EXTRAPOLATION", hasVelocityData, blendedValue.UnityEngine_Quaternion);
                    }
            ...
            if (customValueBlending != null)
            {
                ...
                if (result)
                {
                    ...
                    if (blendedValue.GONetSyncType == GONetSyncableValueTypes.UnityEngine_Quaternion)
                    {
                        VelocitySyncTelemetry.TraceClientBlend(gonetParticipant.GONetId, index, "INTERPOLATION", hasVelocityData, blendedValue.UnityEngine_Quaternion);
                    }
                }
                return result;
            }
    ...
    internal static class GONetParticipant_AutoMagicalSyncCompanion_Generated_Factory
    {
        ...
        /// <summary>
        /// Maps CodeGenerationId to the number of sync values for that type.
        /// Populated by generated code at startup. Used for smart pool sizing.
        /// </summary>
        internal static readonly Dictionary<byte, byte> ValuesCountByCodeGenerationId = new Dictionary<byte, byte>();
        ...
        {
            var companion = theRealness(gonetParticipant);

            // Lazily populate ValuesCountByCodeGenerationId on first creation of each type
            // This enables smart pool drain rate sizing based on scene metadata
            byte codeGenId = companion.CodeGenerationId;
            if (!ValuesCountByCodeGenerationId.ContainsKey(codeGenId))
            {
                ValuesCountByCodeGenerationId[codeGenId] = companion.valuesCount;
            }
        ...
        /// <summary>
        /// Gets the number of sync values for a given CodeGenerationId.
        /// Returns 0 if the CodeGenerationId is not found.
        /// </summary>
        internal static byte GetValuesCountForCodeGenerationId(byte codeGenerationId)
        {
            return ValuesCountByCodeGenerationId.TryGetValue(codeGenerationId, out byte count) ? count : (byte)0;
        }

        /// <summary>
        /// Calculates the expected total sync values for a scene based on DesignTimeMetadata.
        /// This is used for smart pool drain rate sizing at scene load time.
        /// </summary>
        /// <param name="sceneName">The name of the scene (without path)</param>
        /// <returns>Expected total sync values, or 0 if metadata not available</returns>
        internal static int GetExpectedSyncValuesForScene(string sceneName)
        {
            var metadataLibrary = GONetSpawnSupport_Runtime.GetCachedMetadataLibrary();
            if (metadataLibrary?.Entries == null || string.IsNullOrEmpty(sceneName))
                return 0;

            string scenePrefix = string.Concat(GONetSpawnSupport_Runtime.SCENE_HIERARCHY_PREFIX, sceneName, "/");
            int totalValues = 0;

            for (int i = 0; i < metadataLibrary.Entries.Length; i++)
            {
                var entry = metadataLibrary.Entries[i];
                if (entry.Location.StartsWith(scenePrefix))
                {
                    totalValues += GetValuesCountForCodeGenerationId(entry.CodeGenerationId);
                }
            }
