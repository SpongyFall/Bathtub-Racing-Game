...
namespace GONet.Generation
{
    ...
    public class ProcessBuildHelper : IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        ...
        public void OnPreprocessBuild(BuildReport report)
        {
            ...
            GONetSpawnSupport_DesignTime.ClearAllDeferredScanFlags();
            ...
            //GONetLog.Debug($"~~~~~~~~~~~~GEEPs start  BUILD did this!"); // COMMENTED - spammy log (log cleanup)
        ...
        {
            // FAST ITERATION MODE: Skip deletion - code will persist until editor quits
            // This matches the play mode exit behavior when Fast Iteration Mode is enabled
            if (GONet.Editor.GONetProjectSettings.IsFastIterationModeEnabled)
            {
                GONetLog.Debug("[GONet Fast Iteration] Skipping post-build code deletion - code will persist until editor quits.");
            }
            else
            {
        ...
        {
            //GONetLog.Debug($"~~~~~~~~~~~~GEEPs end  BUILD did this!  report.summary.result: {report.summary.result}"); // COMMENTED - spammy log (log cleanup)
    ...
    public static class GONetParticipant_AutoMagicalSyncCompanion_Generated_Generator
    {
        ...
        #region Compilation Gate & Session State Tracking

        /// <summary>
        /// SessionState key used to persist the "pending play mode" flag across domain reloads.
        /// When code is generated during ExitingEditMode, we cancel play mode entry and set this flag.
        /// After the domain reloads (compilation complete), [DidReloadScripts] checks this flag and
        /// verifies the generated types exist before resuming play mode.
        /// </summary>
        private const string SESSION_STATE_PENDING_PLAY_MODE_KEY = "GONet.PendingPlayModeAfterGeneration";

        /// <summary>
        /// SessionState key used to indicate that the user has already acknowledged the dirty warning
        /// for this play mode session. This prevents the warning from showing twice when the compilation
        /// gate defers and then resumes play mode.
        /// </summary>
        private const string SESSION_STATE_DIRTY_WARNING_ACKNOWLEDGED_KEY = "GONet.DirtyWarningAcknowledged";
        private const double PENDING_PLAY_MODE_TIMEOUT_SECONDS = 30.0;
        private static double pendingPlayModeStartTime = -1;
        private static bool pendingPlayModeTimeoutLogged;

        /// <summary>
        /// A known generated type name used to verify that code generation compilation has completed.
        /// This type is defined in BobWad_Generated.cs and must exist for GONet to function.
        /// </summary>
        private const string VERIFICATION_TYPE_NAME = "GONet.SyncEvent_GONetParticipant_GONetId";

        /// <summary>
        /// [PUBLIC API] Check if the dirty warning has already been acknowledged for this play mode session.
        /// Used by GONetSpawnSupport_DesignTime to avoid showing the warning twice.
        /// </summary>
        public static bool WasDirtyWarningAcknowledged => SessionState.GetBool(SESSION_STATE_DIRTY_WARNING_ACKNOWLEDGED_KEY, false);

        /// <summary>
        /// [PUBLIC API] Mark that the dirty warning has been acknowledged for this play mode session.
        /// Called after the user clicks "Proceed Anyway" on the dirty warning dialog.
        /// </summary>
        public static void MarkDirtyWarningAcknowledged()
        {
            SessionState.SetBool(SESSION_STATE_DIRTY_WARNING_ACKNOWLEDGED_KEY, true);
        }

        /// <summary>
        /// Clears the dirty warning acknowledged flag. Called when entering edit mode.
        /// </summary>
        private static void ClearDirtyWarningAcknowledged()
        {
            if (SessionState.GetBool(SESSION_STATE_DIRTY_WARNING_ACKNOWLEDGED_KEY, false))
            {
                SessionState.EraseBool(SESSION_STATE_DIRTY_WARNING_ACKNOWLEDGED_KEY);
            }
        }

        /// <summary>
        /// Called after scripts are reloaded (which happens after compilation completes).
        /// If we have a pending play mode request and the generated types are verified to exist,
        /// we resume entering play mode.
        /// </summary>
        [UnityEditor.Callbacks.DidReloadScripts]
        private static void OnScriptsReloaded_CheckPendingPlayMode()
        {
            TryResumePendingPlayModeAfterGeneration(isFromScriptReload: true);
        }

        internal static void TryResumePendingPlayModeAfterGenerationOnFocus()
        {
            TryResumePendingPlayModeAfterGeneration(isFromScriptReload: false);
        }

        private static void TryResumePendingPlayModeAfterGeneration(bool isFromScriptReload)
        {
            // Check if we have a pending play mode request from before the domain reload
            if (!SessionState.GetBool(SESSION_STATE_PENDING_PLAY_MODE_KEY, false))
            {
                return;
            }

            // Verify the generated types actually exist now
            if (!AreGeneratedTypesAvailable())
            {
                SessionState.EraseBool(SESSION_STATE_PENDING_PLAY_MODE_KEY);
                // This shouldn't happen if we got here via [DidReloadScripts], but handle it gracefully
                GONetLog.Error("[GONet] Generated types still not available after script reload. " +
                    "This may indicate a compilation error. Please check the Console for errors and try again.");
                return;
            }

            if (!GONetSpawnSupport_DesignTime.IsEditorActive())
            {
                if (!GONetSpawnSupport_DesignTime.TryEnablePlayModeWhenUnfocused(out string overrideDetails))
                {
                    if (isFromScriptReload)
                    {
                        GONetLog.Warning("[GONet] Generated code compilation verified, but editor is not focused. " +
                            "Deferring play mode entry until focus returns.");
                    }
                    return;
                }
                ...
                if (isFromScriptReload)
                {
                    string detailsSuffix = string.IsNullOrEmpty(overrideDetails) ? string.Empty : $" ({overrideDetails})";
                    GONetLog.Warning("[GONet] Editor not focused; play-unfocused override enabled" +
                        detailsSuffix + ". Continuing play mode entry.");
                }
            }

            // Clear the flag now that we're resuming play mode
            SessionState.EraseBool(SESSION_STATE_PENDING_PLAY_MODE_KEY);
            if (isFromScriptReload)
            {
                GONetLog.Info("[GONet] Generated code compilation verified. Resuming play mode entry...");
            }
            else
            {
                GONetLog.Warning("[GONet] Editor regained focus; resuming pending play mode entry after compilation.");
            }
            ...
            {
                GONetLog.Info("[GONet] Attempting immediate play mode entry while unfocused (post-compilation resume).");
                GONetSpawnSupport_DesignTime.StartPlayModeRequestWatchdog("post-compilation resume (immediate)");
                EditorApplication.isPlaying = true;
            }

            // Use delayCall to ensure Unity is fully ready before entering play mode
            EditorApplication.delayCall += () =>
            {
                GONetLog.Debug("[GONet] Running delayed play mode resume check (post-compilation).");
                if (!EditorApplication.isPlaying &&
                    !EditorApplication.isPlayingOrWillChangePlaymode &&
                    !EditorApplication.isCompiling)
                {
                    GONetLog.Info("[GONet] Attempting delayed play mode entry (post-compilation).");
                    GONetSpawnSupport_DesignTime.StartPlayModeRequestWatchdog("post-compilation resume");
                    EditorApplication.isPlaying = true;
                }
                else
                {
                    GONetLog.Debug(
                        $"[GONet] Skipping delayed play mode entry (isPlaying={EditorApplication.isPlaying}, " +
        ...
        private static void PendingPlayModeWatchdogUpdate()
        {
            if (!SessionState.GetBool(SESSION_STATE_PENDING_PLAY_MODE_KEY, false))
            {
                pendingPlayModeStartTime = -1;
                pendingPlayModeTimeoutLogged = false;
                return;
            }

            if (pendingPlayModeStartTime < 0)
            {
                pendingPlayModeStartTime = EditorApplication.timeSinceStartup;
            }

            if (!pendingPlayModeTimeoutLogged &&
                EditorApplication.timeSinceStartup - pendingPlayModeStartTime >= PENDING_PLAY_MODE_TIMEOUT_SECONDS)
            {
                pendingPlayModeTimeoutLogged = true;
                bool editorFocused = GONetSpawnSupport_DesignTime.IsEditorActive();
                if (!editorFocused)
                {
                    GONetLog.Warning(
                        $"[GONet] Play mode still pending after {PENDING_PLAY_MODE_TIMEOUT_SECONDS:0} seconds because the editor is not focused. " +
                        $"(EditorApplication.isCompiling={EditorApplication.isCompiling}). Refocus Unity to continue.");
                }
                else
                {
                    GONetLog.Error(
                        $"[GONet] Play mode still blocked after {PENDING_PLAY_MODE_TIMEOUT_SECONDS:0} seconds " +
                        $"(EditorApplication.isCompiling={EditorApplication.isCompiling}, editorFocused={editorFocused}). " +
                        "Check the Console for compile errors and try again.");
                }
            }
        }

        /// <summary>
        /// Verifies that the generated types from BobWad_Generated.cs are compiled and available.
        /// </summary>
        /// <returns>True if the verification type can be resolved, indicating generated code is ready.</returns>
        private static bool AreGeneratedTypesAvailable()
        {
            // NOTE: Type.GetType() without assembly name only searches the calling assembly and mscorlib.
            // Since this code is in an Editor assembly but the generated types are in Assembly-CSharp (runtime),
            // we need to search all loaded assemblies to find the type.
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                Type verificationType = assembly.GetType(VERIFICATION_TYPE_NAME);
        ...
        /// <summary>
        /// Sets the pending play mode flag and cancels the current play mode entry.
        /// Called after GenerateFiles() to defer play mode until after compilation completes.
        /// </summary>
        private static void DeferPlayModeUntilCompilationComplete()
        {
            SessionState.SetBool(SESSION_STATE_PENDING_PLAY_MODE_KEY, true);
            pendingPlayModeStartTime = EditorApplication.timeSinceStartup;
            pendingPlayModeTimeoutLogged = false;
            EditorApplication.isPlaying = false;
            GONetLog.Info("[GONet] Code generation complete. Waiting for compilation before entering play mode...");
        }

        /// <summary>
        /// Clears any pending play mode request and the dirty warning acknowledged flag.
        /// Called when play mode is cancelled or when entering edit mode to ensure clean state.
        /// </summary>
        private static void ClearPendingPlayMode()
        {
            if (SessionState.GetBool(SESSION_STATE_PENDING_PLAY_MODE_KEY, false))
            {
                SessionState.EraseBool(SESSION_STATE_PENDING_PLAY_MODE_KEY);
                GONetLog.Debug("[GONet] Cleared pending play mode request.");
            }
            ClearDirtyWarningAcknowledged();
        }
        ...
        static GONetParticipant_AutoMagicalSyncCompanion_Generated_Generator()
        {
            ...
            EditorApplication.update -= PendingPlayModeWatchdogUpdate;
            EditorApplication.update += PendingPlayModeWatchdogUpdate;
        }
        ...
        internal static void DeleteGeneratedFiles()
        {
            ...
            */
            ...
            for (int i = 0; i < guids.Length; ++i)
            {
                ...
                // Skip persistent generated files that should not be deleted
                if (assetPath.Contains(nameof(SyncEvent_GeneratedTypes)))
                {
                    continue;
                }
                if (assetPath.Contains("AnimatorTriggerHashes"))
                {
                    continue;
                }
            ...
            var filesToDeleteArray = filesToDelete.ToArray();
            List<string> failedToDeleteFiles = new List<string>();
            if (!AssetDatabase.DeleteAssets(filesToDeleteArray, failedToDeleteFiles))
            {
                Debug.LogError($"[GONet Build Error] Failed to delete generated files: {string.Join(", ", failedToDeleteFiles)}");
            }
        ...
        {
            // Filter out GONet's own Generation folder to avoid unnecessary processing
            bool ShouldIgnoreAsset(string assetPath)
            {
                return assetPath != null && assetPath.Contains("/GONet/Code/GONet/Generation/");
            }
            ...
            {
                if (ShouldIgnoreAsset(importedAsset)) continue;
            ...
            {
                if (ShouldIgnoreAsset(deletedAsset)) continue;
            ...
            if (movedAssets != null && movedFromAssetPaths != null) for (int i = 0; i < movedAssets.Length; i++)
            {
                ...
                if (ShouldIgnoreAsset(movedAsset) || ShouldIgnoreAsset(movedFrom)) continue;
        ...
        internal static void OnEditorPlayModeStateChanged_BlockEnteringPlaymodeIfUniqueSnapsChanged(PlayModeStateChange state, out bool didPreventEnteringPlaymode)
        {
            ...
            // FAST ITERATION MODE: When enabled, skip play mode code generation/deletion.
            // Code is generated on editor startup and deleted on editor quit instead.
            // This saves 5-15 seconds per play mode cycle for advanced users who understand the risks.
            bool isFastIterationMode = GONetProjectSettings.IsFastIterationModeEnabled;
            ...
            */
            ...
            {
                // FAST ITERATION MODE: Skip SNAP comparison and generation.
                // When enabled, the user has explicitly opted out of these safety checks.
                // Code is generated on editor startup and deleted on editor quit instead.
                if (isFastIterationMode)
                {
                    // Still verify the generated types are available
                    if (!AreGeneratedTypesAvailable())
                    {
                        GONetLog.Warning("[GONet Fast Iteration] Generated types not available; regenerating now to unblock play mode.");
                        try
                        {
                            // Use default refresh=true for proper compilation ordering
                            UpdateAllUniqueSnaps();
                            GenerateFiles();
                        }
                        catch (Exception ex)
                        {
                            GONetLog.Error("[GONet Fast Iteration] Failed to regenerate code. " +
                                $"Check the Console for details. Error: {ex.Message}");
                        ...
                        if (!AreGeneratedTypesAvailable())
                        {
                            DeferPlayModeUntilCompilationComplete();
                            didPreventEnteringPlaymode = true;
                        }
                        return;
                    }

                    GONetLog.Debug("[GONet Fast Iteration] Skipping SNAP comparison and play mode code generation - using cached code from editor startup.");
                    return;
                }

                // Normal mode: Generate code on play mode entry
                GenerateFiles();

                // CRITICAL FIX: Ensure generated code is compiled before entering play mode.
                // GenerateFiles() calls AssetDatabase.Refresh() which starts compilation, but doesn't wait for it.
                // If we allow play mode to proceed immediately, GONetEventBus.InitializeEventMap() will fail
                // because Type.GetType() returns null for the not-yet-compiled generated types.
                //
                // Solution: Check if the generated types are already available (e.g., if code didn't change).
                // If not, defer play mode entry until after the domain reload that follows compilation.
                // The [DidReloadScripts] callback OnScriptsReloaded_CheckPendingPlayMode() will then
                // verify the types exist and resume play mode.
                if (!AreGeneratedTypesAvailable())
                {
                    DeferPlayModeUntilCompilationComplete();
                    didPreventEnteringPlaymode = true;
                }
                // else: Types are already available (no compilation needed), allow play mode to proceed normally
            }
            else if (state == PlayModeStateChange.EnteredEditMode)
            {
                // Clear any pending play mode flag when returning to edit mode
                ClearPendingPlayMode();

                // FAST ITERATION MODE: Skip deletion - code will be deleted on editor quit instead
                if (isFastIterationMode)
                {
                    GONetLog.Debug("[GONet Fast Iteration] Skipping play mode exit code deletion - code will persist until editor quits.");
                    return;
                }
        ...
        private static void GONetParticipant_EditorOnlyReset(GONetParticipant gonetParticipant)
        {
            ...
            if (AnimationEditorUtils.TryGetAnimatorControllerParameters(animator, out var parameters))
            {
                if (parameters != null && parameters.Length > 0)
                {
                    ...
                    // Detect which parameters are controlled by animation curves (e.g., IK parameters)
                    // These should default to NOT synced because:
                    ...
                    for (int i = 0; i < parameters.Length; ++i)
                    {
                        ...
                        // Note: We add ALL parameter types (including Trigger) to animatorSyncSupport so they appear
                        // in the inspector. Triggers are shown as disabled with a tooltip explaining to use SetAnimatorTrigger().
                        // The actual code generation (see ProcessAllGONetParticipants_AnimatorControllerParameters) skips triggers.
                        ...
                        {
                            // NEW parameter: default isSyncd to false for all params; user enables what they want
                            // Curve-controlled params are marked so inspector can show appropriate warning
                        ...
                        {
                            // EXISTING parameter: update curve-controlled status (animator may have changed)
                            // but preserve user's isSyncd choice - respect their explicit decision
                            gonetParticipant.animatorSyncSupport[parameterSyncMap_key].isCurveControlled = isCurveControlled;
                        }
                    }
                }
            }
        ...
        internal static void UpdateAssetsSnaps(IEnumerable<string> ensureToIncludeTheseGONetParticipantAssets_paths = default)
        {
...

            {
                ...
                try
                {
                    ...
                    foreach (var snapFromAssetFolder in newSnapsFromAssetFolders.OrderBy(x => x.SingleMemberCount).ThenBy(GetDeterministicSnapIdentifier))
                    {
                        bool found = false;
        ...
        /// <summary>
        /// Returns a deterministic string identifier for a SNAP based on its component type names.
        /// This is used as a secondary sort key to ensure stable ordering when multiple SNAPs
        /// have the same SingleMemberCount, preventing non-deterministic code generation IDs.
        /// </summary>
        private static string GetDeterministicSnapIdentifier(GONetParticipant_ComponentsWithAutoSyncMembers snap)
        {
            if (snap?.ComponentMemberNames_By_ComponentTypeFullName == null ||
                snap.ComponentMemberNames_By_ComponentTypeFullName.Length == 0)
            {
                return string.Empty;
            }

            // Create a deterministic identifier from the sorted component type names and their members
            var sb = new System.Text.StringBuilder();
            foreach (var single in snap.ComponentMemberNames_By_ComponentTypeFullName.OrderBy(s => s.componentTypeFullName ?? string.Empty))
            {
                sb.Append(single.componentTypeFullName ?? string.Empty);
                sb.Append(":");
                if (single.autoSyncMembers != null)
                {
                    foreach (var member in single.autoSyncMembers.OrderBy(m => m.memberName ?? string.Empty))
                    {
                        sb.Append(member.memberName ?? string.Empty);
                        sb.Append(",");
                    }
                }
                sb.Append(";");
        ...
        /// Also generates AnimatorTriggerHashes for pre-computed trigger parameter hash constants.
        /// </summary>
        /// <param name="allUniqueSnaps"></param>
        private static void GenerateSyncEventEnum(List<GONetParticipant_ComponentsWithAutoSyncMembers> allUniqueSnaps, List<GONetParticipant> knownGONetParticipants = null)
        {
            Stopwatch sw = Stopwatch.StartNew();
            SyncEvent_GeneratedTypes_Generator.GenerateEnum(allUniqueSnaps);
            GONetLog.Info($"[GONet Perf] GenerateSyncEventEnum: SyncEvent_GeneratedTypes_Generator.GenerateEnum: {sw.ElapsedMilliseconds}ms");

            sw.Restart();
            // PERF OPTIMIZATION: Pass pre-collected GONetParticipants to avoid re-scanning all prefabs
            AnimatorTriggerHashes_Generator.GenerateHashes(knownGONetParticipants);
            GONetLog.Info($"[GONet Perf] GenerateSyncEventEnum: AnimatorTriggerHashes_Generator.GenerateHashes: {sw.ElapsedMilliseconds}ms");
        }
        ...
        {
            Stopwatch generateFilesStopwatch = Stopwatch.StartNew();
            ...
            */
            ...
            GONetLog.Info($"[GONet Perf] GenerateFiles: LoadAllSnapsFromPersistenceFile ({allUniqueSnaps?.Count ?? 0} snaps): {phaseStopwatch.ElapsedMilliseconds}ms");
            ...
            { //Generate one GONetParticipant_AutoMagicalSyncCompanion_Generated file per unique snap.
                ...
                GONetLog.Info($"[GONet Perf] GenerateFiles: GenerateClass loop ({count} classes): {phaseStopwatch.ElapsedMilliseconds}ms");
            }

            phaseStopwatch.Restart();
            GenerateAllRpcCompanions();
            GONetLog.Info($"[GONet Perf] GenerateFiles: GenerateAllRpcCompanions: {phaseStopwatch.ElapsedMilliseconds}ms");

            // GONet v2: Generate SoA descriptor with exact stream capacities
            phaseStopwatch.Restart();
            GONet_SoA_Descriptor_Generator.GenerateSoADescriptor(allUniqueSnaps);
            GONetLog.Info($"[GONet Perf] GenerateFiles: GenerateSoADescriptor: {phaseStopwatch.ElapsedMilliseconds}ms");
            ...
            {//Generate the BobWad_Generated.cs file.
                ...
                GONetLog.Info($"[GONet Perf] GenerateFiles: BobWad_Generated_Generator: {phaseStopwatch.ElapsedMilliseconds}ms");
            }
            ...
            GONetLog.Info($"[GONet Perf] GenerateFiles: AssetDatabase.SaveAssets/Refresh: {phaseStopwatch.ElapsedMilliseconds}ms");

            GONetParticipant.isGenerating = false;
            GONetLog.Info($"[GONet Perf] GenerateFiles TOTAL: {generateFilesStopwatch.ElapsedMilliseconds}ms");
        }
        ...
        /// </summary>
        /// <param name="shouldBypassChangePlaymodeCheck">When true, allows execution even during play mode transitions.</param>
        /// <param name="shouldRefreshAssetDatabase">When true, refreshes the asset database at the end of the process.</param>
        /// <param name="shouldSkipInitialRefresh">When true, skips the expensive initial AssetDatabase.Refresh(ForceUpdate) at the start.
        /// Use this for Fast Iteration Mode startup where Unity has already imported assets during editor startup.
        /// WARNING: Only set to true when you know the asset database is already current (e.g., editor just started).</param>
        public static void UpdateAllUniqueSnaps(bool shouldBypassChangePlaymodeCheck = false, bool shouldRefreshAssetDatabase = true, bool shouldSkipInitialRefresh = false)
        {
            ...
            //GONetLog.Debug($"~~~~~~~~~~~~GEEPs start"); // COMMENTED - spammy log (log cleanup)

            // PERF: Timing instrumentation to identify bottlenecks
            ...
            try
            {
                ...
                // Force asset database to recognize any pending changes
                // NOTE: For Fast Iteration Mode startup, this can be skipped since Unity already imported assets during editor startup.
                // This is the MOST EXPENSIVE operation in the entire generation process due to ForceUpdate forcing full reimport.
                if (!shouldSkipInitialRefresh)
                {
                    phaseStopwatch.Restart();
                    AssetDatabase.SaveAssets();
                    AssetDatabase.Refresh(ImportAssetOptions.ForceUpdate);
                    GONetLog.Info($"[GONet Perf] Initial ForceUpdate refresh: {phaseStopwatch.ElapsedMilliseconds}ms");
                }
                ...
                { // the folling replaces this: UpdateAssetsSnaps();
                    ...
                    GONetLog.Info($"[GONet Perf] GatherGONetParticipantsInAllResourcesFolders ({gnpsInProjectResources?.Count ?? 0} prefabs): {phaseStopwatch.ElapsedMilliseconds}ms");
                    ...
                    GONetLog.Info($"[GONet Perf] EnsureDesignTimeLocationsCurrent_AddressablesOnly: {phaseStopwatch.ElapsedMilliseconds}ms");
                    ...
                    GONetLog.Info($"[GONet Perf] Process prefabs and ensure GUIDs ({snapsFromProjectAssetResourcesFolders.Count} snaps): {phaseStopwatch.ElapsedMilliseconds}ms");

                    //SnapComparer snapComparer = SnapComparer.Instance;
                    //Create the allUniqueSnaps list in order to update codegen ids later
                    phaseStopwatch.Restart();
                    List<GONetParticipant_ComponentsWithAutoSyncMembers> allUniqueSnaps = LoadAllSnapsFromPersistenceFile(GENERATED_IN_SCENE_UNIQUE_SNAPS_FILE_PATH);
                    GONetLog.Info($"[GONet Perf] LoadAllSnapsFromPersistenceFile (scene snaps, {allUniqueSnaps?.Count ?? 0} entries): {phaseStopwatch.ElapsedMilliseconds}ms");

                    phaseStopwatch.Restart();
                    int snapComparisonCount = 0;
                    foreach (var snapFromAssetFolder in snapsFromProjectAssetResourcesFolders.OrderBy(x => x.SingleMemberCount).ThenBy(GetDeterministicSnapIdentifier))
                    {
                        bool found = false;
                        foreach (var uniqueSnap in allUniqueSnaps)
                        {
                    ...
                    GONetLog.Info($"[GONet Perf] Snap comparison loop ({snapComparisonCount} comparisons): {phaseStopwatch.ElapsedMilliseconds}ms");

                    //if (howDeepIsYourSaveStack <= 1) // gotta be careful we do not get into an endless cycle as the method we are in now is called when scene saved.
                    {
                        // persist ALL....
                        phaseStopwatch.Restart();
                        var allDtm = GONetSpawnSupport_Runtime.GetAllDesignTimeMetadata().ToList();
                        foreach (var dtm in allDtm)
                        {
                            GONetSpawnSupport_DesignTime.EnsureExistsInPersistence_WithTheseValues(dtm);
                        }
                        GONetLog.Info($"[GONet Perf] EnsureExistsInPersistence loop ({allDtm.Count} entries): {phaseStopwatch.ElapsedMilliseconds}ms");
                        //GONetLog.Debug($"~~~~~~~~~~~~~~~~GEEPs PROJECT howDeepIsYourSaveStack: {howDeepIsYourSaveStack}, DTM.Count: {GONetSpawnSupport_Runtime.GetAllDesignTimeMetadata().Count()}, all locations: {string.Join("\n", GONetSpawnSupport_Runtime.GetAllDesignTimeMetadata().Select(x => x.Location))}"); // COMMENTED - spammy log (log cleanup)
                    }

                    phaseStopwatch.Restart();
                    SaveUniqueSnapsToPersistenceFile(snapsFromProjectAssetResourcesFolders, ASSET_FOLDER_SNAPS_FILE);
                    GONetLog.Info($"[GONet Perf] SaveUniqueSnapsToPersistenceFile (asset folder snaps): {phaseStopwatch.ElapsedMilliseconds}ms");
                }
                ...
                {//Load all unique snaps (From both the in scenes file and the in assets folder file) and clear out gen ids
                    ...
                    GONetLog.Info($"[GONet Perf] LoadAllSnapsFromPersistenceFile (reload scene snaps): {phaseStopwatch.ElapsedMilliseconds}ms");
                    ...
                    GONetLog.Info($"[GONet Perf] Merge scene snaps to master list ({loadedSnapsFromScenes?.Count ?? 0} checked, {buildingAllSnapsMasterList.Count} total): {phaseStopwatch.ElapsedMilliseconds}ms");

                    phaseStopwatch.Restart();
                    loadedSnapsFromAssetsFolder = LoadAllSnapsFromPersistenceFile(ASSET_FOLDER_SNAPS_FILE);
                    GONetLog.Info($"[GONet Perf] LoadAllSnapsFromPersistenceFile (asset folder snaps): {phaseStopwatch.ElapsedMilliseconds}ms");
                    ...
                    GONetLog.Info($"[GONet Perf] Merge asset folder snaps to master list ({loadedSnapsFromAssetsFolder?.Count ?? 0} checked, {buildingAllSnapsMasterList.Count} total): {phaseStopwatch.ElapsedMilliseconds}ms");
                }
                ...
                GONetLog.Info($"[GONet Perf] CreateAllPossibleUniqueSnapsAndGNPsFromResources: {phaseStopwatch.ElapsedMilliseconds}ms");
                ...
                GONetLog.Info($"[GONet Perf] CreateAllPossibleUniqueSnapsAndGNPsFromLoopingBuildScenes ({buildSceneCount} scenes): {phaseStopwatch.ElapsedMilliseconds}ms");
                ...
                GONetLog.Info($"[GONet Perf] CreateAllPossibleUniqueSnapsAndGNPsFromAddressablesScenes: {phaseStopwatch.ElapsedMilliseconds}ms");
#endif
                ...
                {//Apply changes (Additions, deletes and/or modifications) to both the unique snaps list and to the corresponding in scene game objects
                    ...
                    foreach (var uniqueSnap in buildingAllSnapsMasterList.OrderBy(x => x.SingleMemberCount).ThenBy(GetDeterministicSnapIdentifier))
                    {
                    ...
                    foreach (SnapGnpAssignment inSceneSnapGnpAssignment in createdInSceneUniqueAssignments)
                    {
                        ...
                        // NOTE: Debug log removed - was called 88+ times during generation (log cleanup)
                    ...
                    foreach (SnapGnpAssignment inProjectResourcesAssetsAssignmentMatch in createdInProjectResourcesAssetsUniqueAssignments)
                    {
                        ...
                        // NOTE: Debug log removed - was called for each project resource assignment (log cleanup)
                        DesignTimeMetadata dtm =
...
#endif
                    GONetLog.Info($"[GONet Perf] Apply changes block (snap merge, codegen id assignment, DTM persistence): {phaseStopwatch.ElapsedMilliseconds}ms");
                }
                ...
                //GONetLog.Debug($"dreetsi enum"); // COMMENTED - spammy log (log cleanup)
                phaseStopwatch.Restart();
                // PERF OPTIMIZATION: Pass pre-collected GONetParticipants to avoid re-scanning all prefabs in AnimatorTriggerHashes
                GenerateSyncEventEnum(buildingAllSnapsMasterList, gnpsInProjectResources);
                GONetLog.Info($"[GONet Perf] GenerateSyncEventEnum: {phaseStopwatch.ElapsedMilliseconds}ms");
                ...
                GONetLog.Info($"[GONet Perf] SaveUniqueSnapsToPersistenceFile (scene snaps): {phaseStopwatch.ElapsedMilliseconds}ms");
                phaseStopwatch.Restart();
                SaveUniqueSnapsToPersistenceFile(buildingAllSnapsMasterList, GENERATED_ALL_UNIQUE_SNAPS_FILE_PATH);
                GONetLog.Info($"[GONet Perf] SaveUniqueSnapsToPersistenceFile (all unique snaps): {phaseStopwatch.ElapsedMilliseconds}ms");
                ...
                GONetLog.Info($"[GONet Perf] Final AssetDatabase.SaveAssets/Refresh: {phaseStopwatch.ElapsedMilliseconds}ms");

                GONetLog.Info($"[GONet Perf] UpdateAllUniqueSnaps TOTAL: {overallStopwatch.ElapsedMilliseconds}ms");
            }
            finally
            {
                GONetSpawnSupport_Runtime.IsNewDtmForced = wasNewDtmForced;
            }
            //GONetLog.Debug($"~~~~~~~~~~~~GEEPs end ( no more updates to DTM.json! )"); // COMMENTED - spammy log (log cleanup)
        }
                ...
                foreach (string guid in prefabGUIDs)
                {
                    ...
                    if (prefab != null)
                    {
                        ...
                        if (gnps.Length > 0)
                        {
                            ...
                            if (!GONetProjectSettings.ShouldSuppressPrefabDiscoveryLogging())
                            {
            ...
            out List<SnapGnpAssignment> allSceneSnapGnpAssignments)
        {
            ...
            Stopwatch sceneStopwatch = new Stopwatch();
            long totalSceneLoadMs = 0;
            long totalSceneProcessMs = 0;
            long totalSceneCloseMs = 0;
            ...
            for (int i = 0; i < scenesCount; ++i)
            {
                ...
                long loadMs = sceneStopwatch.ElapsedMilliseconds;
                totalSceneLoadMs += loadMs;
                ...
                long processMs = sceneStopwatch.ElapsedMilliseconds;
                totalSceneProcessMs += processMs;

                // Unload the scene to free up memory, unless it was already open or is the initial active scene
                sceneStopwatch.Restart();
                if (!wasAlreadyLoaded && openScene != initialActiveScene)
                {
                    EditorSceneManager.CloseScene(openScene, true);
                }
                long closeMs = sceneStopwatch.ElapsedMilliseconds;
                totalSceneCloseMs += closeMs;

                // Log per-scene timing for slow scenes (>500ms) to help identify bottlenecks
                long totalSceneMs = loadMs + processMs + closeMs;
                if (totalSceneMs > 500)
                {
                    GONetLog.Info($"[GONet Perf] Scene {i}/{scenesCount} '{System.IO.Path.GetFileNameWithoutExtension(scenePath)}': load={loadMs}ms, process={processMs}ms ({gnpCount} GNPs), close={closeMs}ms, TOTAL={totalSceneMs}ms");
                }
            }

            // Summary timing for all scene processing
            GONetLog.Info($"[GONet Perf] Scene loop summary ({scenesCount} scenes): totalLoad={totalSceneLoadMs}ms, totalProcess={totalSceneProcessMs}ms, totalClose={totalSceneCloseMs}ms");
    ...
    List<MethodInfo> baseRpcMethods, List<MethodInfo> derivedRpcMethods)
        ...
        private static void GenerateRpcHandlerRegistration(StringBuilder sb, MethodInfo method, string className)
        {
            ...
            else if (returnsValue)
            {
                ...
                // CRITICAL: Cache values BEFORE await - pooled events/envelopes are returned after handler yields!
                // The RpcEvent is returned to pool via ISelfReturnEvent.Return() immediately after Handle() returns,
                // but async handlers yield before completing. By the time we resume, CorrelationId is reset to 0.
                sb.AppendLine("                // CRITICAL: Cache before await - pooled events reset after handler yields");
                sb.AppendLine("                long correlationId = rpcEvent.CorrelationId;");
                sb.AppendLine("                ushort sourceAuthorityId = envelope.SourceAuthorityId;");
                sb.AppendLine();
                ...
                sb.AppendLine("                if (correlationId != 0 && sourceAuthorityId != GONetMain.MyAuthorityId)");
                sb.AppendLine("                {");
                sb.AppendLine("                    int bytesUsed;");
                sb.AppendLine("                    bool needsReturn;");
                ...
                sb.AppendLine();
                sb.AppendLine("                    eventBus.Publish(response, targetClientAuthorityId: sourceAuthorityId, shouldPublishReliably: true);");
            ...
            if (targetAttr != null && !string.IsNullOrEmpty(targetAttr.ValidationMethodName))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // NOTE: Debug log removed - was called for each validation method (log cleanup)
                        // Valid signature detection continues without logging
                    }
                }
            }
    ...
    public partial class GONetParticipant_ComponentsWithAutoSyncMembers_SingleMember
    {
        ...
        internal void ApplyProfileToAttribute_IfAppropriate(Type syncMemberType, string memberName)
        {
            ...
            {
                // ANIMATOR PARAMETER TYPE-SPECIFIC PROFILE OVERRIDE:
                // For animator parameters, we automatically select the appropriate profile based on the parameter's data type.
                // - Float params (continuous values like Vertical, Horizontal): use unreliable + blending profile
                // - Int/Bool params (discrete states like State, Mode, Grounded): use reliable + immediate profile
                // This override happens at generation time, so existing cached snaps work correctly.
                if (!string.IsNullOrEmpty(animatorControllerParameterName))
                {
                    // CRITICAL: Clone the attribute for animator params because all animator params from the same
                    // Animator component share the same attribute instance (from GetCustomAttribute on "parameters" property).
                    // Without cloning, modifying one param's profile settings affects ALL params.
                    attribute = new GONetAutoMagicalSyncAttribute(attribute.SettingsProfileTemplateName)
                    {
                        Reliability = attribute.Reliability,
                        ShouldBlendBetweenValuesReceived = attribute.ShouldBlendBetweenValuesReceived,
                        MustRunOnUnityMainThread = attribute.MustRunOnUnityMainThread,
                        ProcessingPriority = attribute.ProcessingPriority,
                        QuantizeDownToBitCount = attribute.QuantizeDownToBitCount,
                        QuantizeLowerBound = attribute.QuantizeLowerBound,
                        QuantizeUpperBound = attribute.QuantizeUpperBound,
                        ShouldSkipSync_RegistrationId = attribute.ShouldSkipSync_RegistrationId,
                    };

                    // Use animatorControllerParameterTypeFullName (the actual animator param type: float/int/bool)
                    // NOT memberTypeFullName (which would be the "parameters" property type)
                    if (animatorControllerParameterTypeFullName == typeof(float).FullName)
                    {
                        // Float = continuous value, use unreliable + blending (smooth interpolation)
                        attribute.SettingsProfileTemplateName = GONetAutoMagicalSyncAttribute.PROFILE_TEMPLATE_NAME___ANIMATOR_CONTROLLER_PARAMETERS_FLOAT;
                    }
                    else if (animatorControllerParameterTypeFullName == typeof(int).FullName || animatorControllerParameterTypeFullName == typeof(bool).FullName)
                    {
                        // Int/Bool = discrete state, use reliable + immediate (no blending between states)
                        attribute.SettingsProfileTemplateName = GONetAutoMagicalSyncAttribute.PROFILE_TEMPLATE_NAME___ANIMATOR_CONTROLLER_PARAMETERS_DISCRETE;
                    }
                }
                ...
                if (profile.SyncValueTypeSerializerOverrides != null && profile.SyncValueTypeSerializerOverrides.Length > 0)
                {
                    ...
                    if (gonetSyncTypeByRealTypeMap.TryGetValue(syncMemberType, out gonetSyncType))
                    {
                        if (profile.SyncValueTypeSerializerOverrides.Any(x => x.ValueType == gonetSyncType))
                        {
                            ...
                            // NOTE: Debug log removed - was called 57+ times during generation (log cleanup)
                            // CustomSerialize_Type is already set on the attribute above
                        }
                    }
                    else
                    {
                ...
                if (profile.SyncValueTypeValueBlendingOverrides != null && profile.SyncValueTypeValueBlendingOverrides.Length > 0)
                {
                    ...
                    if (gonetSyncTypeByRealTypeMap.TryGetValue(syncMemberType, out gonetSyncType))
                    {
                        if (profile.SyncValueTypeValueBlendingOverrides.Any(x => x.ValueType == gonetSyncType))
                        {
                            ...
                            // NOTE: Debug log removed - was called for each value blending type (log cleanup)
                            // CustomValueBlending_Type is already set on the attribute above
                        }
                    }
                    else
                    {
        ...
        private static GONetAutoMagicalSyncSettings_ProfileTemplate LoadProfileAsset(string profileName)
        {
            ...
            if (profile != null)
            {
                // NOTE: Success log removed during performance optimization (log cleanup) - was called 82+ times during generation
                return profile;
            }
    ...
    public partial class GONetParticipant_ComponentsWithAutoSyncMembers
    {
        ...
        /// <summary>
        /// LEGACY: Generic animator parameters attribute. Kept for backward compatibility with the intrinsicAttributeByMemberTypeMap.
        /// Actual animator parameter generation now uses type-specific attributes (float vs discrete) below.
        /// </summary>
        ...
        /// <summary>
        /// Attribute for Float-type animator parameters.
        /// Float params (e.g., Vertical, Horizontal, Speed) are continuous values that benefit from:
        /// - Unreliable transport (lower latency, packet loss acceptable since new values arrive frequently)
        /// - Blending/interpolation on receiving end (smooths out network jitter)
        /// - Regular frequency updates (20Hz)
        /// </summary>
        static readonly GONetAutoMagicalSyncAttribute attribute_animator_parameters_float = new GONetAutoMagicalSyncAttribute(GONetAutoMagicalSyncAttribute.PROFILE_TEMPLATE_NAME___ANIMATOR_CONTROLLER_PARAMETERS_FLOAT);

        /// <summary>
        /// Attribute for Int and Bool-type animator parameters.
        /// Discrete params (e.g., State, Mode, Grounded, IsJumping) are state values that require:
        /// - Reliable transport (cannot miss state transitions - missing Grounded=false causes visual glitches)
        /// - Immediate application (no blending - you're in State 1 or State 2, not State 1.5)
        /// - ASAP frequency (state changes should propagate immediately)
        /// </summary>
        static readonly GONetAutoMagicalSyncAttribute attribute_animator_parameters_discrete = new GONetAutoMagicalSyncAttribute(GONetAutoMagicalSyncAttribute.PROFILE_TEMPLATE_NAME___ANIMATOR_CONTROLLER_PARAMETERS_DISCRETE);
        ...
        internal static readonly Dictionary<ValueTuple<Type, Type>, GONetAutoMagicalSyncAttribute> intrinsicAttributeByMemberTypeMap = new Dictionary<ValueTuple<Type, Type>, GONetAutoMagicalSyncAttribute>(2)
        {
            ...
            { ValueTuple.Create(typeof(Animator), typeof(AnimatorControllerParameter[])), attribute_animator_parameters }, // NOTE: This is legacy; actual animator param generation uses type-specific attributes
        };
            ...
            { // intrinsic Animator properties that cannot manually have the [GONetAutoMagicalSync] added.... (e.g., transform rotation and position)
                if (parameters != null && parameters.Length > 0)
                {
                    ...
                    for (int i = 0; i < parameters.Length; ++i)
                    {
                        ...
                        // Skip Trigger parameters for value-based code generation - Unity has no Animator.GetTrigger() method.
                        // Triggers are synced via event-based system: use GONetParticipant.SetAnimatorTrigger() instead.
                        if (animatorControllerParameter.type == AnimatorControllerParameterType.Trigger)
                        {
                            continue;
                        }

                        // Generate ALL animator parameters - runtime isSyncd toggling handled by ShouldSkipSync delegate
                        if (gonetParticipant.animatorSyncSupport.ContainsKey(animatorControllerParameter.name))
                        {
                            string methodSuffix;
                            string typeFullName;
                            GONetAutoMagicalSyncAttribute animatorParamAttribute;

                            // ANIMATOR PARAMETER TYPE-SPECIFIC SYNC SETTINGS:
                            // - Float params: Continuous values (Vertical, Horizontal, Speed) use unreliable + blending
                            // - Int/Bool params: Discrete states (State, Mode, Grounded) use reliable + immediate
                            // See GONetAutoMagicalSyncAttribute for detailed rationale.
                            switch (animatorControllerParameter.type)
                            {
                                case AnimatorControllerParameterType.Bool:
                                    methodSuffix = "Bool";
                                    typeFullName = typeof(bool).FullName;
                                    // Bool = discrete state, use reliable + immediate (no blending between true/false)
                                    animatorParamAttribute = attribute_animator_parameters_discrete;
                                    break;
                                case AnimatorControllerParameterType.Float:
                                    methodSuffix = "Float";
                                    typeFullName = typeof(float).FullName;
                                    // Float = continuous value, use unreliable + blending (smooth interpolation)
                                    animatorParamAttribute = attribute_animator_parameters_float;
                                    break;
                                case AnimatorControllerParameterType.Int:
                                    methodSuffix = "Integer";
                                    typeFullName = typeof(int).FullName;
                                    // Int = discrete state, use reliable + immediate (no blending between states 1 and 2)
                                    animatorParamAttribute = attribute_animator_parameters_discrete;
                                    break;

                                default:
                                    // This shouldn't happen since we skip Triggers above
                                    continue;
                            }

                            component_autoSyncMembers_animator_parameter.Add(new GONetParticipant_ComponentsWithAutoSyncMembers_SingleMember(animator_parameters, animatorParamAttribute, animatorControllerParameter.nameHash, methodSuffix, typeFullName, animator.runtimeAnimatorController.name, animatorControllerParameter.name));
                        }
                    }
        ...
        internal void ApplyProfileToAttributes_IfAppropriate()
        {
            foreach (GONetParticipant_ComponentsWithAutoSyncMembers_Single single in ComponentMemberNames_By_ComponentTypeFullName)
            {
                ...
                foreach (var singleMember in single.autoSyncMembers)
                {
                    ...
                    Type syncMemberType;
                    if (singleMember.animatorControllerParameterId != 0 && !string.IsNullOrEmpty(singleMember.animatorControllerParameterTypeFullName))
                    {
                        syncMemberType = Type.GetType(singleMember.animatorControllerParameterTypeFullName);
                    }
                    else
                    {
                        syncMemberType = syncMember.MemberType == MemberTypes.Property
                                        ? ((PropertyInfo)syncMember).PropertyType
                                        : ((FieldInfo)syncMember).FieldType;
                    }
