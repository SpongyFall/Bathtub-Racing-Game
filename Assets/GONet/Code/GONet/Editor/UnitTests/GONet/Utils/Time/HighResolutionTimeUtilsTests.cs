...
namespace GONet.Tests.Time
{
    ...
    public class HighResolutionTimeUtilsTests
    {
        ...
        public void Local_And_UTC_Should_Differ_By_Timezone_Offset()
        {
            ...
            // Note: Allow up to 3 seconds drift since HighResolutionTimeUtils uses stopwatch-based
            // elapsed time from an initial anchor point, which can drift from system time
            var utcTime = HighResolutionTimeUtils.UtcNow;
            var systemUtc = DateTime.UtcNow;
            Assert.That(Math.Abs((utcTime - systemUtc).TotalSeconds), Is.LessThan(3),
