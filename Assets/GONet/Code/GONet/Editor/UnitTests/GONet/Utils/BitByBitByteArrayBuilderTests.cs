...
namespace GONet.Utils
{
    ...
    public class BitByBitByteArrayBuilderTests
    {
        ...
        {
            // Clear suppressed patterns to ensure other tests aren't affected
            GONetLog.ClearSuppressedPatterns();

            // Flush any pending logs before test completes
        ...
        [Test]
        [Ignore("DISABLED: Generates 200,000+ warnings (1.8GB+ Editor.log bloat) by intentionally testing extreme float values. " +
                "This test validates BitWriter.WriteFloat() clamping behavior with out-of-range physics values. " +
                "The warnings are expected behavior, but LogAssert.Expect() validates their presence, which conflicts with " +
                "GONetLog.SuppressPattern() used to prevent log bloat. Re-enable if validation of warning messages is critical. " +
                "Suppression mechanism available via: GONetLog.SuppressPattern(BitWriter.LogPattern_FloatBounds)")]
