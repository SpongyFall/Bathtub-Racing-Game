        ...
        public void Should_Handle_Competitive_FPS_Scenario()
        {
            ...
            while (matchStopwatch.Elapsed < TimeSpan.FromSeconds(10))
            {
                ...
                if (matchStopwatch.Elapsed - lastSync > syncInterval)
                {
                    for (int i = 0; i < playerCount; i++)
                    {
                        // CRITICAL: Reset EWMA state before each player's sync
                        // In production, each client has its own static state (separate process).
                        // In this test, all players share static state, so we reset between players
                        // to simulate independent clients.
                        HighPerfTimeSync.ResetForTesting();

                        // Use RawElapsedTicks for request (matches production behavior)
                        var request = new RequestMessage(players[i].RawElapsedTicks);
                        ...
                        HighPerfTimeSync.ProcessTimeSync(
                            request.UID,
                            server.RawElapsedTicks, // Use raw ticks for server too
                            request,
                            players[i],
                            false
                        );
