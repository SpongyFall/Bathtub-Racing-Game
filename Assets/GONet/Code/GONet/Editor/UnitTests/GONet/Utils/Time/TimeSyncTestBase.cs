...
namespace GONet.Tests.Time
{
    ...
    public abstract class TimeSyncTestBase
    {
        ...
        private void ResetAllStaticFields()
        {
            ...
            // Reset failover time sync state in GONetMain
            var gonetMainType = typeof(GONetMain);
            var failoverOffsetField = gonetMainType.GetField("failoverPreservedServerOffset",
                BindingFlags.NonPublic | BindingFlags.Static);
            failoverOffsetField?.SetValue(null, 0L);

            var heartbeatTicksField = gonetMainType.GetField("lastHostHeartbeatElapsedTicks",
                BindingFlags.NonPublic | BindingFlags.Static);
            heartbeatTicksField?.SetValue(null, 0L);

            var heartbeatReceiveField = gonetMainType.GetField("lastHostHeartbeatReceiveRawTicks",
                BindingFlags.NonPublic | BindingFlags.Static);
            heartbeatReceiveField?.SetValue(null, 0L);

            var heartbeatEpochField = gonetMainType.GetField("lastHostHeartbeatEpoch",
                BindingFlags.NonPublic | BindingFlags.Static);
            heartbeatEpochField?.SetValue(null, 0U);

            var heartbeatAuthorityField = gonetMainType.GetField("lastHostHeartbeatAuthorityId",
                BindingFlags.NonPublic | BindingFlags.Static);
            heartbeatAuthorityField?.SetValue(null, (ushort)0);

            var sessionGuidField = gonetMainType.GetField("sessionGUID",
                BindingFlags.NonPublic | BindingFlags.Static);
            sessionGuidField?.SetValue(null, 0L);

            var hostEpochField = gonetMainType.GetField("<HostEpoch>k__BackingField",
                BindingFlags.NonPublic | BindingFlags.Static);
            hostEpochField?.SetValue(null, (uint)0);

            var hostIdentityField = gonetMainType.GetField("<CurrentHostIdentity>k__BackingField",
                BindingFlags.NonPublic | BindingFlags.Static);
            hostIdentityField?.SetValue(null, default(HostIdentity));

            var timeSyncDomainLogField = gonetMainType.GetField("timeSyncDomainLastLogRawTicks",
                BindingFlags.NonPublic | BindingFlags.Static);
            timeSyncDomainLogField?.SetValue(null, 0L);

            var timeSyncDilationLogField = gonetMainType.GetField("timeSyncDilationLastLogRawTicks",
                BindingFlags.NonPublic | BindingFlags.Static);
            timeSyncDilationLogField?.SetValue(null, 0L);
        }
