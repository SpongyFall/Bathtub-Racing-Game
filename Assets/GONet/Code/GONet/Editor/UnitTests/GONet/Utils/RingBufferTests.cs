...
namespace GONet.Tests.Utils
{
    ...
    public class RingBufferTests
    {
        ...
        public void ConcurrentOperations_SingleProducerSingleConsumer_WorksCorrectly()
        {
            ...
            int producerDone = 0; // Use int for Volatile operations (0=false, 1=true)
            ...
            var producer = Task.Run(() =>
            {
                ...
                System.Threading.Volatile.Write(ref producerDone, 1); // Signal completion with memory barrier
            });
            ...
            var consumer = Task.Run(() =>
            {
                int localCount = 0;
                int spinCount = 0;
                const int maxSpins = 1000; // Prevent infinite spin when done
                ...
                while (localCount < itemCount)
                {
                    ...
                    else
                    {
                        ...
                        // Only exit if producer is done AND buffer is empty AND we've spun enough
                        // to ensure no items are in flight
                        if (System.Threading.Volatile.Read(ref producerDone) == 1 &&
                            buffer.Count == 0 &&
                            spinCount > maxSpins)
                        {
                            break; // Producer truly done and buffer confirmed empty
                        }
                    }
                }
