...
{
    [TestFixture]
    [Timeout(30000)] // 30 second timeout - tests have 3 second while loops plus thread operations
    public class TimeCorrectionLogicTests : TimeSyncTestBase
    {
        ...
        public void Should_Handle_Backward_Time_Corrections()
        {
            ...
            // During interpolation, time should never go backward (though it may progress slowly during dilation)
            // Note: During heavy dilation (correcting large offsets), time progression can be very slow,
            // so we check for >= rather than > to avoid flaky failures due to floating-point precision.
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                Assert.That(currentTime, Is.GreaterThanOrEqualTo(previousTime),
                    $"Time should never go backward during interpolation. Previous: {previousTime:F6}, Current: {currentTime:F6}");
                previousTime = currentTime;
            }
            // Verify that time did progress overall (not completely frozen)
            Assert.That(previousTime, Is.GreaterThan(firstTime),
                $"Time should have progressed over the full test duration. First: {firstTime:F3}, Last: {previousTime:F3}");
        }
        ...
        {
            // CRITICAL: Use RawElapsedTicks, not ElapsedTicks!
            // ProcessTimeSync calculates RTT as t2 - t0 where t2 = RawElapsedTicks
            // If we use ElapsedTicks (which includes offset), RTT becomes negative after time sync adjustments
