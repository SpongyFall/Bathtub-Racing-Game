...
namespace GONet
{
    ...
    /// 3. Deferral enabled (default) → bundles queued
    /// 4. Deferral disabled → bundles dropped
    /// 5. Deferral enabled + unreliable → bundles queued (supported)
    ...
    public class GONetSyncBundleDeferralTests
    {
        ...
        [Test]
        public void GONetGlobal_DeferralEnabledByDefault()
        {
            // Assert
            Assert.IsTrue(testGlobal.deferSyncBundlesWaitingForGONetReady,
                "Deferral should be ENABLED by default (prevents spawn/sync races under load)");
        }
        ...
        public void Documentation_DefaultBehaviorExplained()
        {
            ...
            // DEFAULT: deferSyncBundlesWaitingForGONetReady = true
            // BEHAVIOR: Bundles with unready participants → DEFERRED (queued and retried)
            // RATIONALE:
            // - Prevents spawn/sync races under load (sync arriving before spawn processing completes)
            // - Avoids dropping entire bundles when a single participant is missing/unready
            // - Uses bounded FIFO queue + timeout to avoid unbounded memory growth

            Assert.IsTrue(testGlobal.deferSyncBundlesWaitingForGONetReady,
                "Default should be DEFER (prevents spawn/sync races under load)");
        }
        ...
        public void Documentation_WhenToEnableDeferral()
        {
            ...
            // - Dynamic spawning under load (spawn + sync races)
            // - Hot-standby / p2p mesh sessions with frequent reconnects/failovers
            // - Critical state where missing the first sync causes long-lived desync (e.g., spawns, ownership transitions)
            //
            // CONSIDER DISABLING when:
            // - Ultra latency-sensitive action games that can tolerate occasional dropped sync frames
            // - You prefer "drop and recover" behavior via frequent re-sends
        ...
        public void Documentation_PerformanceCharacteristics()
        {
            ...
            // OVERHEAD when deferral ENABLED (default):
