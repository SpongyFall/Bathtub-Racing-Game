...
namespace GONet.Tests.ReliableNetcode
{
    ...
    public class ReliableNetcodeStarvationTests : ReliableNetcodeTestBase
    {
        ...
        public void TestDequeue_RespectsSendBufferAvailableSpace()
        {
            ...
            // NOTE: Lower bound of 10 accounts for timing variations - the 0.5ms time budget may be hit
            // before all 30 slots are filled, since each SendMessage re-counts the entire sendBuffer (~1000 iterations).
            // The key validation is: processed > 1 (old behavior) and processed < 100 (MAX_DEQUEUE limit)
            Assert.GreaterOrEqual(processedCount, 10, $"Should process at least 10 messages (got {processedCount})");
