...
namespace GONet.Tests.ReliableNetcode
{
    ...
    public class UnreliableChannelTests : ReliableEndpointTestBase
    {
        ...
        public void UnreliableChannel_IgnoresDuplicates()
        {
            ...
            pair.Endpoint2.ReceiveCallback = (buffer, length, receiveTimestamp) =>
            {
                // Count every message received (we're only sending unreliable in this test)
                messagesReceived++;
                originalReceive(buffer, length, receiveTimestamp);
            };
        ...
        [Test]
        public void UnreliableChannel_Reset_AllowsSequenceRestart()
        {
            LogTestProgress("Starting UnreliableChannel_Reset_AllowsSequenceRestart test");

            var pair = CreateEndpointPair();

            const double DELTA_TIME = 0.01;

            // Send one unreliable message to establish duplicate-detection state (packet seq=0).
            var msg1 = CreateTestMessage(10000, 100);
            SendTestMessage(pair, pair.Endpoint1, msg1, QosType.Unreliable);
            RunUpdateCycles(pair, 20, DELTA_TIME);

            Assert.AreEqual(1, pair.Endpoint2Received.Count, "Expected first unreliable message to arrive");

            // Simulate failover/reconnect where both endpoints reset their packet sequence counters back to 0.
            // Without clearing the receiver duplicate-detection buffer, the first post-reset packet would be dropped as a DUP.
            pair.Endpoint1.Reset();
            pair.Endpoint2.Reset();

            var msg2 = CreateTestMessage(10001, 100);
            SendTestMessage(pair, pair.Endpoint1, msg2, QosType.Unreliable);
            RunUpdateCycles(pair, 20, DELTA_TIME);

            Assert.AreEqual(2, pair.Endpoint2Received.Count, "Expected post-reset unreliable packet (seq restart) to be delivered");

            LogTestProgress("Test PASSED - Reset clears duplicate detection buffer");
        }
