...
{
    /// <summary>
    /// GONet v2: Position (Vector3) stream with lock-free ring buffers.
    /// Stores history of position updates for temporal blending.
    /// </summary>
    public struct ValueStream_Position : IDisposable
    {
        // Object tracking
        ...
        // Ring buffer - Position X component
        public NativeArray<float> posX;              // [objectIndex][historyIndex]
        public NativeArray<float> posY;
        public NativeArray<float> posZ;
        public NativeArray<long> posTicks;           // Timestamp for each sample
        ...
        public void Initialize(int maxCapacity)
        {
            ...
            GONetLog.Info($"[SoA-INIT] ValueStream_Position.Initialize({maxCapacity}) with ClearMemory fix active");
            ...
            posX = new NativeArray<float>(totalSize, Allocator.Persistent, NativeArrayOptions.ClearMemory);
            posY = new NativeArray<float>(totalSize, Allocator.Persistent, NativeArrayOptions.ClearMemory);
            posZ = new NativeArray<float>(totalSize, Allocator.Persistent, NativeArrayOptions.ClearMemory);
            posTicks = new NativeArray<long>(totalSize, Allocator.Persistent, NativeArrayOptions.ClearMemory);
        ...
        public int RegisterObject(uint gonetId, IntPtr transformPtr)
        {
            ...
            {
                GONetLog.Error($"[SoA] Position stream STILL full after resize! Cannot register GONetId {gonetId} (capacity: {capacity})");
                return -1;
            }
        ...
        public void Resize(int newCapacity)
        {
            ...
            GONetLog.Info($"[SoA-Resize] VECTOR3 stream resizing: {capacity} → {newCapacity} (activeCount: {activeCount})");
            ...
            ResizeArray(ref posX, oldTotalSize, newTotalSize);
            ResizeArray(ref posY, oldTotalSize, newTotalSize);
            ResizeArray(ref posZ, oldTotalSize, newTotalSize);
            ResizeArray(ref posTicks, oldTotalSize, newTotalSize);
        }

        /// <summary>
        /// Helper: Resize a NativeArray (allocate new, copy data, dispose old).
        /// CRITICAL: Uses ClearMemory to zero-initialize new space beyond copied data.
        /// </summary>
