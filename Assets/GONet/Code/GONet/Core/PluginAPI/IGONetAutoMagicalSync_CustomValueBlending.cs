                    ...
                    if (isNewestRecentEnoughToProcess)
                    {
                        ...
                        if (shouldAttemptInterExtraPolation)
                        {
                            ...
                            else // this is the normal case where we can apply interpolation if the settings call for it!
                            {
                                ...
                                if (!didWeLoip)
                                {
                                    //GONetLog.Debug("NEVER NEVER in life did we loip 'eem"); // COMMENTED - spammy log (log cleanup)
                                }
                            }
                        }
                        else
                        {
                            blendedValue = newestValue;
                            //GONetLog.Debug("not a float?"); // COMMENTED - spammy log (log cleanup)
                        }
                    }
                    else
                    {
                        // Data is too old - return false to let the caller handle the stale case.
                        return false;
                    }
                }
        ...
        private void SeeHowWeDid_ExtrapolateQuaternion(NumericValueChangeSnapshot[] valueBuffer, int valueCount)
        {
            if (valueCount > 3)
            {
...

                {
                    ...
                    //GONetLog.Debug($"\naxis_diff_q1_q0: (x:{axis_diff_q1_q0.x}, y:{axis_diff_q1_q0.y}, z:{axis_diff_q1_q0.z}) ang_diff_q1_q0: {ang_diff_q1_q0} ang_diff_q1_q0Scaled: {ang_diff_q1_q0Scaled}\naxis_diff_q2_q1: (x:{axis_diff_q2_q1.x}, y:{axis_diff_q2_q1.y}, z:{axis_diff_q2_q1.z}) ang_diff_q2_q1: {ang_diff_q2_q1} ang_diff_q2_q1Scaled: {ang_diff_q2_q1Scaled}"); // COMMENTED - spammy log (log cleanup)
                    /*
                                        if (ang > 180)
                                        {
                                            ang -= 360;
                ...
                //GONetLog.Debug($"average diff: (x:{average.x}, y:{average.y}, z:{average.z})"); // COMMENTED - spammy log (log cleanup)
            ...
            if (valueCount > 0)
            {
                { // use buffer to determine the actual value that we think is most appropriate for this moment in time
                    ...
                    if (isNewestRecentEnoughToProcess)
                    {
                        ...
                        {
                            blendedValue = newestValue;
                            //GONetLog.Debug("not a quaternion?"); // COMMENTED - spammy log (log cleanup)
                        }
                    }
                    else
                    {
            ...
            if (valueCount > 0)
            {
                { // use buffer to determine the actual value that we think is most appropriate for this moment in time
                    ...
                    if (isNewestRecentEnoughToProcess)
                    {
                        ...
                        {
                            blendedValue = newestValue;
                            //GONetLog.Debug("not a vector2?"); // COMMENTED - spammy log (log cleanup)
                        }
                    }
                    else
                    {
            ...
            if (valueCount > 0)
            {
                { // use buffer to determine the actual value that we think is most appropriate for this moment in time
                    ...
                    if (isNewestRecentEnoughToProcess)
                    {
                        ...
                        {
                            blendedValue = newestValue;
                            //GONetLog.Debug("not a vector4?"); // COMMENTED - spammy log (log cleanup)
                        }
                    }
                    else
                    {
    ...
    public class GONetValueBlending_Float_ExtrapolateWithLowPassSmoothingFilter : IGONetAutoMagicalSync_CustomValueBlending
    {
            ...
            out GONetSyncableValue blendedValue, out bool didExtrapolatePastMostRecentChanges)
        {
            ...
            fixed (NumericValueChangeSnapshot* bufferPtr = valueBuffer)
            {
                ...
                // When data is "too old", return false to let the caller handle the stale case.
                // The caller tracks state and can apply the value once without repeatedly fighting
                // with animation curves that control certain parameters.
            ...
            if (valueCount > 0)
            {
                ...
                if (isNewestRecentEnoughToProcess)
                {
                    ...
                    else
                    {
                        ...
                        if (!didInterpolate)
                        {
                            //GONetLog.Debug("Failed to interpolate quaternion - using newest value"); // COMMENTED - spammy log (log cleanup)
