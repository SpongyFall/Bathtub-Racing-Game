...
namespace GONet.Utils
{
    ...
    public class TieredArrayPool<T>
    {
        ...
        public void Return(T[] borrowed)
        {
            ...
            // Array not found in any pool - this can happen for oversized arrays (>64KB) that were
            // allocated directly by Borrow() when the request exceeded largePool's maximum.
            // These arrays bypass pool tracking and should just be garbage collected.
            // PERF FIX (Dec 2025): Don't throw - just let oversized arrays GC naturally.
            // Previous behavior threw InvalidOperationException for arrays like 113KB chunk reassembly buffers.
            if (borrowed.Length > 65536) // 64KB = largePool maximum
            {
                // Oversized allocation - expected behavior, let GC handle it
                return;
            }

            // If array is within pool size range but not found, that's a real error (double-return or wrong pool)
