...
namespace GONet.Utils
{
    ...
    public static class HighResolutionTimeUtils
    {
        ...
        private static class MonotonicStopwatch
        {
            ...
            /// <summary>
            /// Restarts the stopwatch. CRITICAL for Fast Iteration Mode where Shutdown() stopped it.
            /// </summary>
        ...
        {
            // CRITICAL FIX (Jan 2025): Reset shutdown state and restart stopwatch.
            // In Fast Iteration Mode, Shutdown() stops the MonotonicStopwatch when exiting play mode.
            // Without restarting it here, ElapsedTicks returns a frozen value and time never progresses.
            if (Interlocked.Exchange(ref shutdownState, 0) == 1)
            {
                InitializePlatformTimer();
            }
            MonotonicStopwatch.Restart();
        ...
        public static DateTime UtcNow
        {
            ...
            get
            {
                ...
                if (swTicks < threadLocalLastCheck)
                {
                    threadLocalLastCheck = 0;
                    threadLocalLastLocalTicks = 0;
                    threadLocalLastUtcTicks = 0;
