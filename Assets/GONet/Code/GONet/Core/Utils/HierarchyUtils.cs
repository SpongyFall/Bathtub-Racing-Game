...
namespace GONet.Utils
{
    public static class HierarchyUtils
    {
        ...
        /// <summary>
        /// Returns a unique path from <paramref name="root"/> to <paramref name="target"/> (exclusive of root).
        /// Format matches GetFullUniquePath but omits the scene name and root segment.
        /// Returns empty string if root is null, target is null, or root is not an ancestor of target.
        /// </summary>
        public static string GetFullUniquePathRelativeTo(GameObject root, GameObject target)
        {
            if ((object)root == null || (object)target == null)
            {
                return string.Empty;
            }
            ...
            while (current != null && current != rootTransform)
            {
                ...
                {
                    GONetParticipant gonetParticipant = current.GetComponent<GONetParticipant>();
                    if ((object)gonetParticipant != null && gonetParticipant.DoesGONetIdContainAllComponents())
                    {
                        segment = string.Concat(segment, GONETID_PREFIX, gonetParticipant.GONetIdAtInstantiation, GONETID_SUFFIX);
                    }
                }

                int siblingCount, mySiblingIndex;
                if (DoIHaveSiblingsOfSameName(current.gameObject, out siblingCount, out mySiblingIndex))
                {
                    segment = string.Concat(segment, SAME_NAME_SIBLING_PREFIX, mySiblingIndex, SAME_NAME_SIBLING_SUFFIX);
                }
        ...
        /// <summary>
        /// Finds a GameObject by a unique path relative to <paramref name="root"/>.
        /// Path format is produced by GetFullUniquePathRelativeTo.
        /// Returns null if not found.
        /// </summary>
        public static GameObject FindByRelativeUniquePath(GameObject root, string relativePath)
        {
            if ((object)root == null)
            {
                return null;
            ...
            string[] uniquePathParts = relativePath.Split(PATH_SEPARATOR_CHAR);
            string sceneName = root.scene == null ? DONT_DESTROY_ON_LOAD_SCENE : root.scene.name;

            for (int iUniquePart = 0; iUniquePart < uniquePathParts.Length; ++iUniquePart)
            {
                ...
                if (uniquePathPart.EndsWith(SAME_NAME_SIBLING_SUFFIX))
                {
                    ...
                    current = GetUniqueSibling(sceneName, current, gameObjectNameOriginal, uniqueSiblingIndex, hasGONetIdAtInstantiation, gonetIdAtInstantiation);
                }
                else
                {
                    if (current == null)
                    ...
                    Transform child = current.transform.Find(uniquePathPart);
                    if (child == null)
                    {
                        return null;
                    }
                ...
                if (hasGONetIdAtInstantiation && current != null)
                {
                    GONetParticipant gonetParticipant = current.GetComponent<GONetParticipant>();
                    if ((object)gonetParticipant == null || (gonetParticipant.GONetIdAtInstantiation != gonetIdAtInstantiation && !GONetMain.WasDefinedInScene(gonetParticipant)))
                    {
                        ...
                        GONetLog.Warning($"[HIERARCHY-MISMATCH] WRONG GameObject found by relative path lookup!\n" +
                                       $"  Root: '{root.name}'\n" +
