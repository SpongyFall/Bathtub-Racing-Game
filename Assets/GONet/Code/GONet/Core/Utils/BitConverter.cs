...
namespace GONet.Utils
{
    ...
    public unsafe static class BitConverter
    {
        ...
        public static void GetBytes(byte value, byte[] destination, int offset = 0)
        {
            if (destination == null)
            {
            ...
            if (offset < 0 || (offset + sizeof(byte) > destination.Length))
            {
        ...
        public static void GetBytes(sbyte value, byte[] destination, int offset = 0)
        {
            if (destination == null)
            {
                throw new ArgumentNullException(nameof(destination));
            ...
            if (offset < 0 || (offset + sizeof(sbyte) > destination.Length))
            {
                throw new ArgumentOutOfRangeException(nameof(offset));
            }
