...
{
    /// <summary>
    /// GONet v2: Vector2 stream with lock-free ring buffers.
    /// Stores history of Vector2 updates for temporal blending.
    /// </summary>
    public struct ValueStream_Vector2 : IDisposable
    {
        // Object tracking
        ...
        // Ring buffer - Vector2 components (X, Y)
        ...
        public int RegisterObject(uint gonetId, byte memberIndex, IntPtr companionPtr)
        {
            ...
            {
                GONetLog.Error($"[SoA] Vector2 stream STILL full after resize! Cannot register GONetId {gonetId} (capacity: {capacity})");
                return -1;
            }
        ...
        public void Resize(int newCapacity)
        {
            ...
            GONetLog.Info($"[SoA-Resize] VECTOR2 stream resizing: {capacity} → {newCapacity} (activeCount: {activeCount})");
