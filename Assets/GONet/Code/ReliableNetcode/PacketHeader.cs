...
namespace ReliableNetcode
{
    ...
    internal class ReceivedPacketData
    {
	    ...
	    internal class FragmentReassemblyData
	    {
	        ...
	        public void StoreFragmentData(byte channelID, uint sessionId, ushort sequence, ushort ack, uint ackBits, int fragmentID, int fragmentSize, byte[] fragmentData, int fragmentBytes)
	        {
	            int copyOffset = 0;

	            if (fragmentID == 0) {
	                byte[] packetHeader = BufferPool.GetBuffer(Defines.MAX_PACKET_HEADER_BYTES);
	                int headerBytes = PacketIO.WritePacketHeader(packetHeader, channelID, sessionId, sequence, ack, ackBits);
