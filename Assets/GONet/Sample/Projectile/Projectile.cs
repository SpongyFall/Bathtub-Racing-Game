...
namespace GONet.Sample
{
    ...
    public class Projectile : GONetParticipantCompanionBehaviour
    {
        ...
        private bool hasLoggedStuckOnce = false;
        private float despawnTimer = 0f;
        private float stuckDiagnosticTimer = 0f;
        private const float STUCK_DIAGNOSTIC_INTERVAL = 5f; // Log every 5 seconds if stuck

        protected override void OnBorrowFromPool()
        {
            base.OnBorrowFromPool();

            // Reset runtime state for pooled reuse.
            speed = startSpeed;
            despawnTimer = 0f;
            stuckDiagnosticTimer = 0f;
            hasLoggedOwnershipOnce = false;
            hasLoggedStuckOnce = false;

            // Movement direction should match the borrow rotation.
            movementDirection = transform.forward;
        ...
        protected override void OnReturnToPool()
        {
            ...
            // Clear transient state while in the pool.
            despawnTimer = 0f;
            stuckDiagnosticTimer = 0f;
            hasLoggedOwnershipOnce = false;
            hasLoggedStuckOnce = false;
        ...
        private void Update()
        {
            if (gonetParticipant.IsMine)
            {
                ...
                else
                {
                    ...
                    {
                        GONetMain.ReturnToPool(gonetParticipant);
                    }
                    else
                    {
            ...
            else if (gonetParticipant.IsLocallyResponsible)
            {
                // FAILOVER CLEANUP: Server takes responsibility for orphaned projectiles.
                // Since IsMine is false but IsLocallyResponsible is true, we're the server and the
                // owner client is no longer connected. See GONetParticipant.IsLocallyResponsible docs.
                if (gonetParticipant.IsPooled)
                {
                    GONetLog.Debug($"[Projectile] Server returning orphaned pooled projectile '{gameObject.name}' (GONetId: {gonetParticipant.GONetId}) - " +
                        $"owner authority {gonetParticipant.OwnerAuthorityId} not connected");
                    GONetMain.ReturnToPool(gonetParticipant);
                }
                else
                {
                    GONetLog.Debug($"[Projectile] Server destroying orphaned projectile '{gameObject.name}' (GONetId: {gonetParticipant.GONetId}) - " +
                        $"owner authority {gonetParticipant.OwnerAuthorityId} not connected");
                    Destroy(gameObject);
                }
            }
            else
            {
                ...
                // DIAGNOSTIC: Log ONCE when entering stuck state (commented out - development diagnostic)
                // if (!hasLoggedStuckOnce)
                // {
                //     hasLoggedStuckOnce = true;
                //     bool isServer = GONetMain.IsServer;
                //     bool ownerConnected = isServer ? GONetMain.Server_IsClientOwnerConnected(gonetParticipant) : false;
                //     GONetLog.Warning($"[STUCK-PROJECTILE-FIRST] GONetId={gonetParticipant.GONetId}, " +
                //         $"OwnerAuth={gonetParticipant.OwnerAuthorityId}, " +
                //         $"MyAuth={GONetMain.MyAuthorityId}, " +
                //         $"IsMine={gonetParticipant.IsMine}, " +
                //         $"IsLocallyResponsible={gonetParticipant.IsLocallyResponsible}, " +
                //         $"IsServer={isServer}, " +
                //         $"OwnerConnected={ownerConnected}, " +
                //         $"HostEpoch={GONetMain.HostEpoch}");
                // }

                // DIAGNOSTIC: Log stuck projectile info periodically (commented out - development diagnostic)
                // stuckDiagnosticTimer += Time.deltaTime;
                // if (stuckDiagnosticTimer >= STUCK_DIAGNOSTIC_INTERVAL)
                // {
                //     stuckDiagnosticTimer = 0f;
                //     bool isServer = GONetMain.IsServer;
                //     bool ownerConnected = isServer ? GONetMain.Server_IsClientOwnerConnected(gonetParticipant) : false;
                //     GONetLog.Warning($"[STUCK-PROJECTILE] GONetId={gonetParticipant.GONetId}, " +
                //         $"OwnerAuth={gonetParticipant.OwnerAuthorityId}, " +
                //         $"MyAuth={GONetMain.MyAuthorityId}, " +
                //         $"IsMine={gonetParticipant.IsMine}, " +
                //         $"IsLocallyResponsible={gonetParticipant.IsLocallyResponsible}, " +
