...
public class ClaimableObjectViaRpcs : GONetParticipantCompanionBehaviour
{
    ...
    {
        // INSTRUMENTATION (Dec 2025): Log click details
        GONetLog.Info($"[CLICK] OnMouseDown! GONetId={GONetParticipant?.GONetId ?? 0}, ClaimedBy={ClaimedByAuthorityId}, isClaimedByMe={isClaimedByMe}, MyAuth={GONetMain.MyAuthorityId}, Time={GONetMain.Time?.ElapsedSeconds ?? 0:F2}s");
        ...
        {
            // Use CallRpcAsync for RPCs with return values
            GONetLog.Info($"[CLICK] Sending RequestClaim RPC... Time={GONetMain.Time?.ElapsedSeconds ?? 0:F2}s");
            ClaimResult result = await CallRpcAsync<ClaimResult>(nameof(RequestClaim));
            GONetLog.Info($"[CLICK] RequestClaim RPC returned! Success={result.Success}, Time={GONetMain.Time?.ElapsedSeconds ?? 0:F2}s");
    ...
    internal async Task<ClaimResult> RequestClaim()
    {
        ...
        // INSTRUMENTATION (Dec 2025): Log server-side RPC execution
        GONetLog.Info($"[RPC-SERVER] RequestClaim executing! GONetId={GONetParticipant?.GONetId ?? 0}, Time={GONetMain.Time?.ElapsedSeconds ?? 0:F2}s");
