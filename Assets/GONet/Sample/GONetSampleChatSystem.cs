...
public class GONetSampleChatSystem : GONetParticipantCompanionBehaviour
{
    ...
    public override void OnGONetReady()
    {
        ...
        // TEMP DEBUG: Check if OnGONetReady is called and what values we get
        GONetLog.Info($"[CHAT-ONGONETREADY] OnGONetReady() called - GONetMain.MyAuthorityId={GONetMain.MyAuthorityId}, localAuthorityId={localAuthorityId}, IsServer={GONetMain.IsServer}, gonetParticipant.OwnerAuthorityId={gonetParticipant?.OwnerAuthorityId}");
        ...
        if (!participants.Any(p => p.AuthorityId == localAuthorityId))
        {
            ...
            // Removed spammy CHAT-DEBUG logging
    ...
    void OnGUI()
    {
        // PERFORMANCE: Early exit if UI is globally disabled (Left Shift + X toggle)
    ...
    private void SendCurrentMessage()
    {
        ...
        // TEMP DEBUG: Investigate missing client messages
        GONetLog.Info($"[CHAT-SEND-DEBUG] SendCurrentMessage ENTERED - content: '{currentInputText}', localAuthorityId: {localAuthorityId}, IsServer: {GONetMain.IsServer}");
        ...
        switch (currentChatMode)
        {
            case ChatType.Channel:
                ...
                GONetLog.Info($"[CHAT-SEND-DEBUG] Channel mode: participants.Count={participants.Count}, uniqueTargets.Count={uniqueTargets.Count}");
                break;
            ...
            case ChatType.GroupMessage:
                ...
                GONetLog.Info($"[CHAT-SEND-DEBUG] DM/Group mode: selectedParticipants.Count={selectedParticipants.Count}, uniqueTargets.Count={uniqueTargets.Count}");
                break;
        }

        CurrentMessageTargets = uniqueTargets.ToList();
        GONetLog.Info($"[CHAT-SEND-DEBUG] CurrentMessageTargets set: Count={CurrentMessageTargets.Count}, targets=[{string.Join(",", CurrentMessageTargets)}]");
        ...
        GONetLog.Info($"[CHAT-SEND-DEBUG] About to CallRpcAsync for SendMessage...");
        CallRpcAsync<RpcDeliveryReport, string, string, ChatType, ushort, ushort[]>(
            ...
            .ContinueWith(task =>
            {
                GONetLog.Info($"[CHAT-SEND-DEBUG] CallRpcAsync ContinueWith - Task.IsCompleted: {task.IsCompleted}, Task.IsFaulted: {task.IsFaulted}");
                if (task.IsFaulted)
                {
                    GONetLog.Error($"[CHAT-SEND-DEBUG] CallRpcAsync FAULTED: {task.Exception}");
                }
                else if (task.IsCompleted)
                {
                    ...
                    {
                        GONetLog.Warning($"[Chat] Failed to deliver to some recipients: {task.Result.FailureReason}");
                    }
                    else
                    {
                        GONetLog.Info($"[CHAT-SEND-DEBUG] Message delivered successfully!");
                    }
                }
            });

        currentInputText = "";
        GONetLog.Info($"[CHAT-SEND-DEBUG] SendCurrentMessage EXITING");
    }
