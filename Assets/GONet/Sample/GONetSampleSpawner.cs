...
public class GONetSampleSpawner : MonoBehaviourGONetCallbacks
{
    ...
    protected override void Awake()
    {
        ...
        // NOTE: Sample UI components (GONetStatusUI, VoluntaryHostMigrationPromptUI) are now
        // initialized by GONetConnectionWizard.EnsureStatusUIExists() and EnsureHostMigrationPromptUIExists()
        // which works for all clients regardless of which scene they join (including late-joiners)
    }
    ...
    private void TurnOffNonAuthorityStuff(GONetParticipant gonetParticipant, CharacterController characterController)
    {
        ...
        // IMPORTANT: Only disable the Camera COMPONENT, not the entire GameObject!
        // Disabling the GameObject would make the entire hierarchy inactive, which causes
        // GONet lifecycle issues for any GONetParticipants parented underneath (e.g., items
        // held in an ItemHoldPoint that's a child of the camera).
        foreach (var camera in gonetParticipant.GetComponentsInChildren<Camera>())
        {
            camera.enabled = false;
        }

        // Also disable AudioListeners on remote players to prevent audio conflicts
        foreach (var audioListener in gonetParticipant.GetComponentsInChildren<AudioListener>())
        {
