...
public class CircularMotion : GONetParticipantCompanionBehaviour
{
    ...
    private bool isInitialized = false; // Only start moving after GONetReady
    ...
    /// <summary>
    /// Called when GONetParticipant is fully ready (GONetId assigned, AllCurrentValues received).
    /// This is the safe place to initialize authority-side randomization and start movement.
    ///
    /// Previously used OnGONetClientVsServerStatusKnown, but that fires BEFORE AllCurrentValues
    /// arrives on non-authority clients, causing objects to appear at wrong positions.
    /// </summary>
    internal override void UpdateAfterGONetReady()
    {
        ...
        {
            // One-time initialization when authority is ready
            if (!isInitialized)
            {
                isInitialized = true;
