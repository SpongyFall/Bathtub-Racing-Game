...
namespace GONet.Sample
{
    ...
    public class SceneSelectionUI : MonoBehaviour
    {
        ...
        private void OnApproveClicked()
        {
            ...
            // CRITICAL: Hide any other approval panels that might be showing (e.g., ExitButtonUI)
            // SceneSelectionUI takes priority and handles all approvals when present
            HideOtherApprovalPanels();
        ...
        /// <summary>
        /// Hides approval panels from other UI systems (e.g., ExitButtonUI, GONetConnectionManager).
        /// Called when SceneSelectionUI handles approval to ensure no duplicate panels remain visible.
        /// </summary>
        private void HideOtherApprovalPanels()
        {
            GONetLog.Info($"[SceneSelectionUI] HideOtherApprovalPanels called");

            // Find ALL approval panels in the scene by name
            GameObject[] allGameObjects = FindObjectsOfType<GameObject>();
            int foundPanels = 0;
            foreach (GameObject go in allGameObjects)
            {
                if (go.name.Contains("Approval") && go.name.Contains("Panel") && go.activeSelf)
                {
                    foundPanels++;
                    GONetLog.Info($"[SceneSelectionUI] Found active approval panel: '{go.name}' (parent: {go.transform.parent?.name ?? "null"})");
                }
            }
            GONetLog.Info($"[SceneSelectionUI] Total active approval panels found in scene: {foundPanels}");

            // Hide ExitButtonUI's approval panel if it exists
            ExitButtonUI exitButtonUI = FindObjectOfType<ExitButtonUI>();
            GONetLog.Info($"[SceneSelectionUI] ExitButtonUI found: {exitButtonUI != null}");

            if (exitButtonUI != null)
            {
                // Use reflection to access ExitButtonUI's private approvalPanel field
                var approvalPanelField = typeof(ExitButtonUI).GetField("approvalPanel",
                    System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                GONetLog.Info($"[SceneSelectionUI] ExitButtonUI approvalPanel field found: {approvalPanelField != null}");

                if (approvalPanelField != null)
                {
                    GameObject exitApprovalPanel = approvalPanelField.GetValue(exitButtonUI) as GameObject;
                    GONetLog.Info($"[SceneSelectionUI] ExitButtonUI approvalPanel value: {(exitApprovalPanel != null ? exitApprovalPanel.name : "null")}, active: {exitApprovalPanel?.activeSelf}");

                    if (exitApprovalPanel != null)
                    {
                        if (exitApprovalPanel.activeSelf)
                        {
                            exitApprovalPanel.SetActive(false);
                            GONetLog.Info($"[SceneSelectionUI] Hid ExitButtonUI's approval panel");
                        }
                        else
                        {
                            GONetLog.Info($"[SceneSelectionUI] ExitButtonUI's approval panel already inactive");
                        }
                    }
                }
            }

            // Hide GONetConnectionManager's approval panel if it exists
            GONetConnectionManager connectionManager = FindObjectOfType<GONetConnectionManager>();
            GONetLog.Info($"[SceneSelectionUI] GONetConnectionManager found: {connectionManager != null}");

            if (connectionManager != null)
            {
                // Use reflection to access GONetConnectionManager's private sceneApprovalPanel field
                var approvalPanelField = typeof(GONetConnectionManager).GetField("sceneApprovalPanel",
                    System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                GONetLog.Info($"[SceneSelectionUI] GONetConnectionManager sceneApprovalPanel field found: {approvalPanelField != null}");

                if (approvalPanelField != null)
                {
                    GameObject connMgrApprovalPanel = approvalPanelField.GetValue(connectionManager) as GameObject;
                    GONetLog.Info($"[SceneSelectionUI] GONetConnectionManager sceneApprovalPanel value: {(connMgrApprovalPanel != null ? connMgrApprovalPanel.name : "null")}, active: {connMgrApprovalPanel?.activeSelf}");

                    if (connMgrApprovalPanel != null)
                    {
                        if (connMgrApprovalPanel.activeSelf)
                        {
                            connMgrApprovalPanel.SetActive(false);
                            GONetLog.Info($"[SceneSelectionUI] Hid GONetConnectionManager's approval panel");
                        }
                        else
                        {
                            GONetLog.Info($"[SceneSelectionUI] GONetConnectionManager's approval panel already inactive");
                        }
                    }
                }
            }
        ...
        private void OnSceneLoadCompleted(string sceneName, LoadSceneMode mode)
        {
            ...
            // CRITICAL: Hide approval panel when scene loads (server approval was granted)
            if (approvalPanel != null && approvalPanel.activeSelf)
            {
                GONetLog.Info($"[SceneSelectionUI] Hiding approval panel on scene load completion");
                approvalPanel.SetActive(false);
                hasPendingRequest = false;
            }

            // Also hide any other approval panels that might be lingering
            HideOtherApprovalPanels();
        }
